...
$target_path="";
	...
	if(isset($_FILES['file']))
			...
			//MtCredit c'est le montent versée par le client
			// MtCreditFournisseur montant n'a pas payé par le fournisseur
  		$CodeReg= "NR".Increment_Chaine_F("CodeReg","reglements","IdReg",$con,"",array());	
			  	$reqInser2 = "INSERT INTO  reglements(CodeReg,DateReg,Heure,MtEspece,MtCheque,MtCredit,MtCreditFournisseur,NumCheque,PhotoCheque
				,BqCheque,NumVirement,CodeBuySel,TypeOperator) 
					values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$params2= array(
					$CodeReg,
					date("Y-m-d"),
					...
					floatval(tofloat($_POST['MtVirement'])),
...
if($error=="" ) {
 sqlsrv_commit( $con );
	...
	//alert(IdReg);
		window.open ('regelements_vdr_pdf.php?IdReg='+IdReg, "_newtab" )
...
<input type="hidden" value="<?php echo $_GET['ListeFac'];?>" dir="ltr" name="ListeFac"  class="form-control" />
	...
	<input type="text" id="TotalReg" value="<?php echo $TotalReg; ?>" name="TotalReg" class="" />
<div class=" row Fac form-group ">
		<div class=" col-md-6">
		<?php echo lang('NameSeller'); ?>
		<span id="Fac"><?php 
...
<div id="Cheque" CLAss=" form-group row  " class="CadreT" style="background:#FBEFF5;">
	...
	<div class=" col-md-12 row">
		...
		<div class=" form-group row">			
			<div class=" col-md-12">
				<input  class="jfilestyle" 
				data-text="Find file"
				type="file" name="file" data-theme="blue" id="file" size="10" tabindex="1" value="" />
			</div>
...
<div id="Virement" CLAss="  row  "  class="CadreT" style="background:#CEECF5;">
	<div class=" col-md-3" ><input type="button" value="<?php  //echo $trad['label']['Credit'];?>" id="BtnVirement"></div>
	<div class=" col-md-9">	<BR>
		<div class=" row ">
	
			<div class=" col-md-2"><label><?php  echo lang('Virement');?> :</label></div>
			<div class=" col-md-3">
			<input type="text"  dir="ltr" value=" " class="form-control" id="MtVirement" name="MtVirement" size="10"
			name="NumVirement" >
			</div>
		<div class=" col-md-2"><label><?php  echo lang('NumVirement');?> :</label></div>
		<div class=" col-md-5"><input type="text" value=" " dir="ltr" name="NumVirement"  class="form-control" /></DIV>
		</div>
	</div>
</div>  


<div id="Credit" CLAss="  row  "  class="CadreT" style="background:#ebebeb;">
	<div class=" col-md-3" ><input type="button" value="<?php  //echo $trad['label']['Credit'];?>" id="BtnCredit"></div>
	<div class=" col-md-9">	<BR>
		<div class=" row ">
	
			<div class=" col-md-2"><label><?php  echo lang('MtCreditFournisseur');?> :</label></div>
			<div class=" col-md-3">
			<input type="text"  dir="ltr" value=" " style="background:#FE2E2E;color:#fff" class="form-control" id="MtCredit" name="MtCredit" size="10" name="Num" readonly>
			</div>
...
$(function () {
	...
	// quand on met le curseur dans l'input on le vide
	 ...
	 $("#MtVirement").unbind().click(function() {
				if(($('#TotalReg').val()!="") && ($('#TotalReg').val()!="0") ){
				$('#MtCredit').val($('#MtVirement').val()); 
				$('#MtVirement').val('');}
	});
...
$.keypad.addKeyDef('UPPER', 'upper', function(inst) { 
	...
	function OnChangeCheque(){
		...
		var count2 = char_count($("#MtCheque").val(), '.');
			...
			if(Reste>0) $("#MtVirement").val(number_format(Reste, 3, ".", " ") ); else $("#MtVirement").val("");
			$("#MtCredit").val("");
			$('#MtVirement').keypad({
				upperText: 'U/C', 
				upperStatus: 'Convert to upper case', 
				...
				onClose: function(value, inst) { 
						...
						OnChangeVirement();	
						$('#MtVirement').val(number_format($('#MtVirement').val(),3,"."," ")); 						
					}
			  });
	}
	
	function OnChangeVirement(){
		var count = char_count($("#MtEspece").val(), '.');
		var count2 = char_count($("#MtCheque").val(), '.');
		var count3 = char_count($("#MtVirement").val(), '.');
			var TotalReg=$("#TotalReg").val().replace(" ","");
			TotalReg=parseFloat( TotalReg.replace(/[^\d\.]/g,'') );
			//$("#MtEspece").val($("#TotalReg").val());
			if(($("#MtCheque").val()!=" ") && ($("#MtCheque").val()!="") &&  (count2<=1)&& ($("#MtCheque").val()!="."))var MtCheque=$("#MtCheque").val().replace(" ","");
			...
			if(($("#MtVirement").val()!=" ") && ($("#MtVirement").val()!="")&& (count<=1)&& ($("#MtVirement").val()!="."))var MtVirement=$("#MtVirement").val().replace(" ","");
			else var MtVirement=0;
			
			if(MtCheque!="") MtCheque=parseFloat( MtCheque.replace(/[^\d\.]/g,'') );
			if(MtEspece!="")  MtEspece=parseFloat( MtEspece.replace(/[^\d\.]/g,'') );
			if(MtVirement!="")  MtVirement=parseFloat( MtVirement.replace(/[^\d\.]/g,'') );
			var Reste=parseFloat(TotalReg)-(parseFloat(MtCheque)+parseFloat(MtEspece)+parseFloat(MtVirement));
	...
	$("#BtnVirement").click(function() {
		var TotalReg=$("#TotalReg").val().replace(" ","");
		TotalReg=parseFloat( TotalReg.replace(/[^\d\.]/g,'') );
		$("#MtVirement").val(number_format(TotalReg, 3, ".", " ") );
		$("#MtCredit").val("");
		$("#MtEspece").val("");
		$("#MtCheque").val("");
...
/*	var files = $('#formReg:input[type=file]').get(0).files;
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'paiement_vdr.php?TerminerReg',
								method			:	'post',
