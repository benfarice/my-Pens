...
/*
	...
	f.Date_F,f.Total,isnull((select
	d.IdReg from detailReglements d where d.IdFac = f.iD_Fac),0) as reg_f ,f.Taxe
	from facture_vendeur f where cast(f.Date_F as date) between '$date_a_'
	and '$date_b_'";
*/
$query_select = "select f.Num_facture,(select bs.name_ar from BUYER_SELLER bs
 where bs.ID_B_S = f.Code_Vendeur) as seller,
f.Date_F,f.Total,isnull((select
d.IdReg from detailReglements d where d.IdFac = f.iD_Fac),0) as reg_f ,f.Taxe
from facture_vendeur f where cast(f.Date_F as date) between  convert(date, '$_REQUEST[date_d_selected]',105) and convert(date, '$_REQUEST[date_f_selected]',105)";
