...
// create new PDF document
  $obj_pdf = new MYPDF('P', PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);  
		...
		$sql = "
		 SELECT df.num_adjudication NumAdj, fac.Num_facture CodeFac,Date_F  DateFac,fac.Total TotalFac,
 a.Code_Acheteur CodeBye ,bye.name_ar NameAcht ,e.Nom_espece NameEsp
   ,a.Prix_unitaire PrixUnite,a.Poids_net TotalPoids, a.Prix_net TotalPrix ,sel.name_ar NameSel,a.num_lot NumLot,
  a.Date_adjudication DateAdj,sel.ID_B_S CodeSel
   FROM facture_vendeur fac 
   INNER JOIN details_facture_vendeur df ON df.Num_facture=fac.Num_facture
   INNER JOIN ADJUDICATION a ON a.Num_adjudication=df.num_adjudication
   INNER JOIN LOT l ON l.Num_lot = a.num_lot
    ...
    INNER JOIN BUYER_SELLER bye ON bye.ID_B_S=a.Code_Acheteur AND bye.TYPE_B_S='BUYER' 
		 INNER JOIN BUYER_SELLER sel ON sel.ID_B_S=fac.Code_vendeur AND sel.TYPE_B_S='SELLER' 
		where fac.Id_Fac like '".$_GET['IdFac']."'  ";
...
//arcourir($params);
			 ...
			 while($row=sqlsrv_fetch_array($resAff)){							 
								/*	  $Options.="<option value=".$row['IdType'].">". stripslashes (htmlentities($row['IdType']."  ".
										...
										if (!isset($groups[$key])) {
											...
											$groups[$key]['CodeSel']=$row['CodeSel'];
											$groups[$key]['NameSeller']=$row['NameSel'];
									...
									//	echo "<br>---".$i."---<br>";
											...
											$groups[$key][$i]['NameAcht']=$row['NameAcht'];
		...
		$html2.='
					<table width="100%" border="1" id="tableID" align="center"  style="border-collapse: collapse; border-color:#000; " >
					  <tr  class="" style=" height:170px;background-color:#ebebeb;color:#000;"  >
									...
									<td  width="20%" align="center">'.lang('NameBye').'</td>
	...
	$CodeFac="";$CodeSel ="";$NameBye="";$DateFac=0;$NbrAdj=0;$TotalValAdj=0;$Tva=5;$Tax=0;$ValObll=0;$ValObllLettre="";
		$nRes = sqlsrv_num_rows($resAff);
	//	echo $nRes;
		...
		//$DateFac=$v['DateFac'];
		$DateFac = $v['DateFac'];//->format('d/m/Y')	;					
//echo $Date;
		...
		foreach($v as $row){
				...
				$html2.='	
						<tr  height="40px" >
								...
								<td align="right"  > &nbsp;&nbsp;'. htmlentities($row['NameBye']).' </td>
	...
	$ValObllLettre = convertNumber($ValObll,false);
		$html2 .= '</table>';

			$html2.='<BR><BR><BR><table width="100%" border="1" id="tableID" align="center" 
			style="border-collapse: collapse; border-color:#000; ">	
						<tr  >
								<td align="right" >'.lang('Total_sell').' ('.lang('reyal_homany').')</td>
								<td align="right"  > '.lang('count_lot').' </td>
								<td align="right"  >'.lang('taxe_sell').'</td>		
								<td align="right"  >'.lang('taxe_value').' ('.lang('reyal_homany').')</td>		
						...
						<tr   >
								...
								<td align="right"  >&nbsp;&nbsp;5%</td>	
								<td align="right"  >&nbsp;&nbsp;'.number_format($Tax, 3, '.', ' ').'</td>
						</tr>
						<tr >
								<td align="right" colspan="3" >'.lang('ValObll').'</td>		
								
								<td align="right"  >&nbsp;&nbsp;'.number_format($ValObll, 3, '.', ' ').'</td>
						</tr>
		...
		$html1.='<table width="100%"  border="0" >
			...
			<td width="90%">
				<table WIDTh="100%"  border="0" align="center" >
					<tr> <td width="28%" align="right">فاتورة البائع رقم </td>
						<TD width="33%">'.$CodeFac.'</td>
						<td width="38%" align="left">Invoice Seller Number	</td>
					</tr>
					<tr> <td align="right"> تاريخ الفاتورة </td>
						<TD>'.$DateFac.'</td>
						<td  align="left">Date	</td>
					</tr>
					<tr> <td align="right">رقم البائع  </td>
						<TD>'.$CodeSel.'</td>
...
// This method has several options, check the source code documentation for more information.
		$name='facture_vdr_pdf '.date('d-m-Y H-i').'.pdf';	
...
var name="<?php echo $name;?>";
	...
	//document.location.href="http://localhost:82/fish/backend/all_file/"+name;
		document.location.href="http://www.pgd.ma/atlas/backend/all_file/"+name;
