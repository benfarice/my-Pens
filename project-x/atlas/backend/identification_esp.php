	...
	//echo toDateSql($_POST['DateF'));return;
		...
		if(isset($_POST['DateD']) && isset($_POST['DateF']) && ($_POST['DateD']!="") && ($_POST['DateF']!="")  )
		{
			...
			{ 
				 ...
				 $where.= " where  convert(date,Date_lot) = convert(date, '".($_POST['DateD'])."',105)";
				 $title=lang('IdentificationEsp')." ".$_POST['DateD'] ;
			}
			else
			{
				 $where.= " where Date_lot between  convert(date, '".($_POST['DateD'])."',105 ) and convert(date,  '".($_POST['DateF'])."',105) ";
				 $title= lang('IdentificationEsp') ." ". lang('From')." ".$_POST['DateD']." ". lang('To')." ".$_POST['DateF'] ;
			}
		}
	/*	else
		{
...
*/
	$sqlA = " 
			...
			SELECT e.Code CodeEspece,e.Nom_espece NameEspece ,sum(l.Poids_net) AS PoidsNet  ,count(l.Num_lot) NbrLot
		FROM 
		espece e
		INNER JOIN LOT l ON l.Code_espece = e.Code_espece
		
		".$where."
		GROUP BY  e.Code,e.Nom_espece ";
//echo $sqlA;
    $params = array();
...
if ((isset($_POST['optradio'])) && ($_POST['optradio']=="Tableau")) {?>
		...
		<div class="responsive-table-line" style="margin:0px auto;max-width:1275px;">
			<table class="table table-bordered table-condensed table-body-center" id="table1">
					<thead class="entete">
						<tr >
						<th><?php echo lang('CodeEspece');?></th>
						<th><?php echo lang('NameEspece');?></th>
				...
				<?php while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){		
					...
					<tr>
						...
						<td data-title="<?php echo lang('CodeEspece');?>"><?php echo ucfirst(stripslashes($row['CodeEspece']));?></td>
						<td data-title="<?php echo lang('NameEspece');?>"><?php  echo ucfirst(stripslashes($row['NameEspece']));?>	</td>
...
$i=0;
	...
	if($i<10){
			$series .="{ name: '".ucfirst(stripslashes($row['NameEspece']))."', y: ".$row['PoidsNet'].", Total: ".$row['PoidsNet']."},";
			$Poids .= " ".$row['PoidsNet'].", ";		
			$noms .= " '".ucfirst(stripslashes($row['NameEspece']))."', ";
				$i=$i+1;
			}
	}
	 ...
	 $('#graphG').highcharts('Chart', {
       ...
       /* tooltip: {
		...
		tooltip: {
			...
			pointFormat: '<span style="color:RED;font-size:16px"> : {point.name}</span> <b> <br/> {point.y:,.3f} <?php echo lang('Kg');?></b> <br/>'
		},
...
exit;
	...
	if(!isset($noNavbar)){
		...
		<div class="container-fluid">
			<br><Center><h2><?php echo lang('IdentificationEsp');?></h2></center>
			   ...
			   <div class="modal-dialog modal-lg" >
				 <div class="modal-content">
					  <div class="modal-header">                   
						<h2 class="modal-title" id="myModalLabel"><?php echo lang('IdentificationEsp');?></h2>
					  </div>
						<div id="Res"></div>
...
rechercher();
	//$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="../images/loading2.gif" /></center>').load('identification_esp.php?aff');
});
function rechercher(){
	
	$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'identification_esp.php?rech'});
		}
 function ajouter(){
		$('#act').attr('value','add');	
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='identification_esp.php?add';
 ...
 function mod(id){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='identification_esp.php?mod&&ID='+id;
	//	alert(url);
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
 ...
 jConfirm('<h3><?php  echo lang('terminerOperation'); ?></h3>', '<?php  echo lang('Confirm'); ?>', function(r) {
					if(r)	{
     if($('#act').val()=="add" ){ 
		var url="identification_esp.php?goAdd";
			 }
    else   if($('#act').val()=="mod" ){ 
		var url="identification_esp.php?goMod";
			 }
	//alert(url);
	// alert(url);
                $.ajax({
