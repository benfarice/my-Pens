...
if (isset($_GET['ListAcht'])){?>
  ...
  and ID_B_S in (SELECT Code_Acheteur FROM ADJUDICATION
...
function	RechAcht(){
	var val=$("#SearchB").val();
$('#Buyer').load('reglement_acht.php?RechAcht&Search='+val);
}
	</script>
<?php
	exit;
	...
	if(isset($_POST['DateD']) && isset($_POST['DateF']) && ($_POST['DateD']!="") && ($_POST['DateF']!="")  )
		{
			...
			{ 
				 ...
				 $where.= " where  cast(date as date)  = convert(date, '".($_POST['DateD'])."',105)";
			
			}
			else
			{
				 $where.= " where  cast(date as date)  between  convert(date, '".($_POST['DateD'])."',105 ) and convert(date,  '".($_POST['DateF'])."',105) ";
...
/*		if(isset($_POST['DateD']) && ($_POST['DateD']!="")  )
			{ 
				 ...
				 $title=lang('reglement_acht')." ".$_POST['DateD'] ;
			}
			*/
		...
		<div class="responsive-table-line" style="margin:0px auto;max-width:1275px;">
			...
			<table class="table table-bordered table-condensed table-body-center" id="table1">
					<thead class="entete">
						...
						<th width="8%" class="chpinvisible"><?php echo lang('CodeBye');?></th>
						<th width="12%" class="chpinvisible"><?php echo lang('NameBye');?></th>
	 ...
	 <div class="pretty p-default p-smooth p-bigge prettyEntete">
				...
				<input type="text" class="chpinvisible" id="NameBye" name="NameBye" value="<?php echo $row['NameBye'];?>" />
					<tr>				
						...
						<td data-title="<?php echo lang('CodeBye');?>" class="chpinvisible"><?php  echo ucfirst(stripslashes($row['CodeBye']));?>	</td>
						<td data-title="<?php echo lang('NameBye');?>" class="chpinvisible"><?php echo ucfirst(stripslashes($row['NameBye']));?></td>
	  ...
	  $('input#TotalReg').attr("value","0.000");
		...
		function TypeReg(){
			...
			//		ajaxindicatorstart('<?php echo lang('patienter');?>');
					var $modal = $('#BoxB');
						//var url='reglement_acht.php?TypeReg';
						var NameBuy=$("#NameBye").val();
						var CodeBuy=$("#CodeBuy").val();
							var url='paiement.php?ListeFac='+idSelect+'&&Type=Buyer&&ChoixTypeReg&&Precommande&&TotalReg='+TotalReg+'&&CodeBuy='+CodeBuy+'&NameBuy='+encodeURIComponent(NameBuy);
...
exit;
	...
	if(!isset($noNavbar)){
		...
		<div class="container-fluid">
			...
			<br><Center><h2><?php echo lang('reglement_acht');?></h2></center>
 ...
 <form id="formRechF" method="post" name="formRechF"  class="bootstrap-iso"> 
           ...
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">                   
                    <h2 class="modal-title" id="myModalLabel"><?php echo lang('reglement_acht');?></h2>
                  </div>
				    <div id="Res"></div>
	 ...
	 aria-hidden="false">
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">                   
                    <h2 class="modal-title" id="myModalLabelB"><?php echo lang('reglement_acht');?></h2>
                  </div>
				    <div id="ResB"></div>
                  <div class="modal-body" style="    text-align: right;">
								<img src="http://conferoapp.com/icons/preloader.gif" class="progress">
...
/*	var files = $('#formReg:input[type=file]').get(0).files;
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'reglement_acht.php?TerminerReg',
								method			:	'post',
...
//rechercher();
	//$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="../images/loading2.gif" /></center>').load('reglement_acht.php?aff');
});
function rechercher(){
	
	if($("#CodeBye").val()=="") {
				jAlert("<h4><?php  echo lang('SaisirAchteur'); ?><h4>","<?php  echo lang('operation_th'); ?>");  

		
	}
	else {$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'reglement_acht.php?rech'});
	}
		}
 function ListAcht(){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='reglement_acht.php?ListAcht';
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
    })
 ...
 function mod(id){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='reglement_acht.php?mod&&ID='+id;
	//	alert(url);
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
 ...
 jConfirm('<h3><?php  echo lang('terminerOperation'); ?></h3>', '<?php  echo lang('Confirm'); ?>', function(r) {
					if(r)	{
     if($('#act').val()=="add" ){ 
		var url="reglement_acht.php?goAdd";
			 }
    else   if($('#act').val()=="mod" ){ 
		var url="reglement_acht.php?goMod";
			 }
	//alert(url);
	// alert(url);
                $.ajax({
