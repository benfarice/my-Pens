...
$Tax=(floatval($_POST['TotalFac'])/100 )*5;
  		$CodeFac= "NF".Increment_Chaine_F("CodeFac","reglement_vdr","IdFac",$con,"",array());	
			  	$reqInser2 = "INSERT INTO  reglement_vdr(CodeFac,Date,Heure,Montant,TypeReg,Taxe,Pourcentage) 
					values (?,?,?,?,?,?,?)";
			$params2= array(
					$CodeFac,
					date("Y-m-d"),
			...
			//---------------------------ID fiche fac--------------------------------//
			$sql = "SELECT max(IdFac) as IdFac FROM reglement_vdr ";
...
if($error=="" ) {
 sqlsrv_commit( $con );
	...
	<script language="javascript" type="text/javascript">
		...
		//document.location.href='facture_vdr_pdf.php?IdFac='+IdFac;
		window.open ('facture_vdr_pdf.php?IdFac='+IdFac, "_newtab" );
	...
	/*	
		 ...
		 options = "Width=900,Height=900" ;
			window.open( 'facture_vdr.print.php?IdFac='+IdFac, "edition", options ) ;*/
	</script>
	<?php
...
if (isset($_GET['ListVdr'])){?>
	<form id="FormAdd" method="post" name="FormAdd"> 
	<div class="row">
	<div class="col-md-10" >
	<label for="user_lastname"><?php echo lang('NameSeller');?></label>
	...
	<div class="col-md-2" >
			...
			onclick="RechVdr()" style="border:none;padding:12px 30px;margin-top:22px">
	</div>
...
function	RechVdr(){
	var val=$("#SearchB").val();
$('#Buyer').load('reglement_vdr.php?RechVdr&Search='+val);
}
	</script>
<?php
	exit;
...
$title="";
	...
	//	echo $where;
	$sqlA = " 		
				SELECT  f.Id_Fac IdFac,f.Num_facture CodeFac ,bye.ID_B_S CodeSeller,
				bye.name_".$_SESSION['Lang']." NameSeller ,f.Total Montant,
					f.Date_F DateFac

				fROM facture_vendeur f
				inner join details_facture_vendeur df on df.num_facture=f.num_facture
				INNER JOIN BUYER_SELLER bye ON bye.ID_B_S=f.Code_vendeur AND bye.TYPE_B_S='SELLER' 	".$where ."
				and f.Id_Fac not in(select dr.IdFac from detailReglements dr)
				 GROUP BY f.Id_Fac ,f.Num_facture  ,bye.ID_B_S ,
				bye.name_ar  ,f.Total,
					f.Date_F 
		";
//echo $sqlA."<hr>";
		...
		<div class="responsive-table-line" style="margin:0px auto;max-width:1275px;">
			...
			<div class="pretty p-default p-smooth p-bigge prettyEntete">
				...
				<?php $i=0; while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){		
					?>
					<input type="text" class="chpinvisible" name="CodeBuy" id="CodeBuy" value="<?php echo $row['CodeSeller'];?>" />
				<input type="text" class="chpinvisible" id="NameBye" name="NameBye" value="<?php echo $row['NameSeller'];?>" />
					<tr>				
						<td data-title="<?php echo lang('num_facture');?>"><?php echo ucfirst(stripslashes($row['CodeFac']));?></td>
						
						<td data-title="<?php echo lang('facture_date_title');?>"><?php 
						$Date = $row['DateFac'];
						echo  $Date;?>
	  ...
	  $('input#TotalReg').attr("value","0.000");
		...
		function TypeReg(){
			...
			//		ajaxindicatorstart('<?php echo lang('patienter');?>');
					var $modal = $('#BoxB');
						//var url='reglement_vdr.php?TypeReg';
						var NameBuy=$("#NameBye").val();
						var CodeBuy=$("#CodeBuy").val();
							var url='paiement_vdr.php?ListeFac='+idSelect+'&&Type=Seller&&ChoixTypeReg&&Precommande&&TotalReg='+TotalReg+'&&CodeBuy='+CodeBuy+'&NameBuy='+encodeURIComponent(NameBuy);
...
exit;
	...
	if(!isset($noNavbar)){
		...
		<div class="container-fluid">
			...
			<br><Center><h2><?php echo lang('reglement_vdr');?></h2></center>
 ...
 <form id="formRechF" method="post" name="formRechF"  class="bootstrap-iso"> 
		<div class="row " >
			...
			<?php echo lang('CodeSeller');?></label>	</div>
			<div  class=" middleLabel   col-md-2 col-sm-12 centerLabel " >
			...
			onclick="ListVdr()"; style="padding:25px" />
           ...
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">                   
                    <h2 class="modal-title" id="myModalLabel"><?php echo lang('reglement_vdr');?></h2>
                  </div>
				    <div id="Res"></div>
	 ...
	 aria-hidden="false">
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">                   
                    <h2 class="modal-title" id="myModalLabelB"><?php echo lang('reglement_vdr');?></h2>
                  </div>
				    <div id="ResB"></div>
                  <div class="modal-body" style="    text-align: right;">
								<img src="http://conferoapp.com/icons/preloader.gif" class="progress">
...
/*	var files = $('#formReg:input[type=file]').get(0).files;
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'reglement_vdr.php?TerminerReg',
								method			:	'post',
...
//rechercher();
	//$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="../images/loading2.gif" /></center>').load('reglement_vdr.php?aff');
});
function rechercher(){
	
	if($("#CodeBye").val()=="") {
				jAlert("<h4><?php  echo lang('SaisirVdreur'); ?><h4>","<?php  echo lang('operation_th'); ?>");  

		
	}
	else {$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'reglement_vdr.php?rech'});
	}
		}
 function ListVdr(){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='reglement_vdr.php?ListVdr';
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
    })
 ...
 function mod(id){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='reglement_vdr.php?mod&&ID='+id;
	//	alert(url);
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
 ...
 jConfirm('<h3><?php  echo lang('terminerOperation'); ?></h3>', '<?php  echo lang('Confirm'); ?>', function(r) {
					if(r)	{
     if($('#act').val()=="add" ){ 
		var url="reglement_vdr.php?goAdd";
			 }
    else   if($('#act').val()=="mod" ){ 
		var url="reglement_vdr.php?goMod";
			 }
	//alert(url);
	// alert(url);
                $.ajax({
