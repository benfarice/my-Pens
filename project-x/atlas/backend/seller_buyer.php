...
?>
	...
	<input type="hidden" value='<?php echo $ID;?>' name="IdTable" />
			   ...
			   <div class="col-md-6 col-sm-12 ">
			   			<div class="row">
			   					<div class="col-md-12 text-right">
			   						<label><?php  echo lang('NomAr');?> :	</label></div>
			    ...
			    <div class="col-md-6 col-sm-12 ">
			    		<div class="row">
			    			<div class="col-md-12 text-right">
			    				<label><?php  echo lang('NomEn');?> :	</label></div>
 	  ...
 	  <div class="row">
			   <div class="col-md-6 col-sm-12 ">
			   			<div class="row">
			   				<div class="col-md-12 text-right ">	<label><?php  echo lang('LicenceNumber');?> :	</label></div>
...
//*************************************
$query_seller_code = "select count(*) as new_code from BUYER_SELLER s where s.TYPE_B_S = 'seller'";



$params__add_seller_code = array();
$options__add__seller_code =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
$result__add_seller_code = sqlsrv_query($con,$query_seller_code,$params__add_seller_code,
$options__add__seller_code);
$new_code=0;
while($reader_add_seller_code= sqlsrv_fetch_array($result__add_seller_code, SQLSRV_FETCH_ASSOC)){
	$new_code = $reader_add_seller_code['new_code'];
}
$new_code++;
...
$query_num_code_2 = "select * from BUYER_SELLER s where s.ID_B_S = '$new_code'";
...
//*************************************
//$Code= "S".Increment_Chaine_F("ID_B_S","BUYER_SELLER","IdSelBuy",$con,"","");

$Code= 'S'.date('y').'00'.$new_code;
...
?>
	<form id="FormAdd" method="post" name="FormAdd">
		  <div class="row  form-group chpinvisible">
					<div class="col-md-12">

								<div class="btn-group" data-toggle="buttons">

									<label class="btn btn-default active form-check-label">
										<input class="form-check-input" type="radio" name="Type" value="SELLER" checked autocomplete="off">
										<?php  echo lang('Vendeur');?>
									</label>
		...
		<div class="row">
			   ...
			   <div class="col-md-6 col-sm-12 ">
						...
						<div class="col-md-12 text-right">	<label><?php  echo lang('NomAr');?> :	</label></div>
			   ...
			   <div class="col-md-6 col-sm-12  ">
			   		<div class="row ">
					<div class="col-md-12 text-right">
						<label><?php  echo lang('NomEn');?> :	</label>
					</div>
					<div class="col-md-12 col-sm-12">
						  <input class="form-control" type="text" name="NomEn"
						   id="NomEn" />
					</div>
					</div>
				</div>
...
*/
	...
	FROM BUYER_SELLER where TYPE_B_S like 'SELLER'
	".$where." " ;
			...
			<table class="table table-bordered table-condensed table-body-center" id="table1">
				...
				<?php while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){
					...
					<tr>
						...
						<td data-title=""><a onclick="mod(<?php echo $row['IdSelBuy'];?>)" class="btn btn-primary btn-lg  colorWhite"  data-toggle="modal" >
...
exit;
	...
	if(!isset($noNavbar)){
		...
		<div class="container-fluid">
				<br><Center><h2><?php echo lang('SellerManage');?></h2></center>
 ...
 <form id="formRechF" method="post" name="formRechF" >
		...
		<div class="row">
			<div  class="col-md-12 col-sm-12 centerLabel text-center" Style="text-align:center">
           ...
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">
                    <h2 class="modal-title" id="myModalLabel"><?php echo lang('SellerManage');?></h2>
                  </div>
				    <div id="Res"></div>
                  <div class="modal-body">
								<img src="layout/images/loading.gif" class="progress">
                  </div>
				  <div class="clear"></div>
...
$(document).ready(function() {
	$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>').load('seller_buyer.php?aff');
});
function rechercher(){

	$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'seller_buyer.php?rech'});
		}
 function ajouter(){
		$('#act').attr('value','add');
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='seller_buyer.php?add';
 ...
 function mod(id){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='seller_buyer.php?mod&&ID='+id;
	//	alert(url);
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
...
if(r)	{
     if($('#act').val()=="add" ){
		var url="seller_buyer.php?goAdd";
			 }
    else   if($('#act').val()=="mod" ){
		var url="seller_buyer.php?goMod";
			 }
	//alert(url);
	// alert(url);
                $.ajax({
