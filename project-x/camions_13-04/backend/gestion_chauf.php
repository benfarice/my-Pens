...
*/
	$sqlA = "SELECT ID Id,c.Nom Nom,c.matricule Matricule, c.CIN CIN,c.date_emb DateEmb FROM chauffeurs c
	".$where." " ;
//echo $sqlA;
    $params = array();
...
exit;
	...
	if(!isset($noNavbar)){
		...
		<div class="container-fluid">
			<br><Center><h2><?php echo lang('gestion_chauf');?></h2></center>
           ...
           <div class="modal-dialog modal-lg" >
             <div class="modal-content">
                  <div class="modal-header">
                    <h2 class="modal-title" id="myModalLabel"><?php echo lang('gestion_chauf');?></h2>
                  </div>
				    <div id="Res"></div>
...
$(document).ready(function() {
	...
	$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>').load('gestion_chauf.php?aff');
});
function rechercher(){

	$('#formRes').html('<center><br/><br/><?php echo lang('patienter');?> <br/><img src="layout/images/loading.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'gestion_chauf.php?rech'});
		}
 function ajouter(){
		$('#act').attr('value','add');
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='gestion_chauf.php?add';
 ...
 function mod(id){
	...
	ajaxindicatorstart('<?php echo lang('patienter');?>');
    var $modal = $('#BoxA');
		var url='gestion_chauf.php?mod&&ID='+id;
	//	alert(url);
     $.get(url, null, function(data) {
      //$modal.find('.modal-body').html(data);
	   $modal.find('.modal-body').html(data);
 ...
 jConfirm('<?php  echo lang('terminerOperation'); ?>', '<?php  echo lang('Confirm'); ?>', function(r) {
					if(r)	{
     if($('#act').val()=="add" ){
		var url="gestion_chauf.php?goAdd";
			 }
    else   if($('#act').val()=="mod" ){
		var url="gestion_chauf.php?goMod";
			 }
	//alert(url);
	// alert(url);
                $.ajax({
