...
if (isset($_GET['rech']) or isset($_GET['aff'])){
		...
		{
		$where=" where cast(f.date AS date)=convert(date, '".(date('d/m/Y'))."',105)";
		}
...
$sqlA = "SELECT v.codeVendeur,v.cin ,v.nom + ' ' + v.prenom AS 'Nom' ,
fa.idFamille,fa.codeFamille,fa.Designation AS famille,g.IdGamme,g.Designation AS
 gamme,a.CB ,a.Reference, a.Designation AS article,df.UniteVente,sum(df.qte) as qte, sum(df.ttc) AS ttc ,
(SELECT count( DISTINCT visite) FROM factures fa WHERE fa.idVendeur=v.idVendeur) as countVisite 
...
INNER JOIN familles fa ON fa.idFamille=s.idFamille".$where ." and Etatcmd=2 ";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	if(isset($_POST['CodeVnd']) && ($_POST['CodeVnd']!='') )
	{	$sqlA .=" AND v.codeVendeur like ? " ;
	   $params = array("%".$_POST['CodeVnd']."%");
	}
	$sqlA .=" GROUP BY v.codeVendeur,v.idVendeur,v.cin,v.nom ,v.prenom ,fa.idFamille,fa.codeFamille,fa.Designation,g.IdGamme,g.Designation,a.CB ,a.Reference,a.Designation,df.UniteVente  ";
	//echo $sqlA	;
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="100%" border="0" cellspacing="2">
       <tr class="entete">
	    <td width="10%"><?php echo $trad['label']['codeVnd']; ?></td>  
        <td width="10%"><?php echo $trad['label']['Vendeur']; ?></td>
		<!--td width="15%"><?php //echo $trad['label']['codeFamille']; ?></td-->
        <td width="15%"><?php echo $trad['label']['Gamme']; ?></td>		
		<td width="10%"><?php echo $trad['label']['reference']; ?></td>
        ...
        <td width="15%"><?php echo $trad['label']['ValTTC']; ?></td>
...
<!--<div id="cList">-->
	...
	//v.cin ,v.nom + ' ' + v.prenom AS 'Nom' ,fa.idFamille,fa.codeFamille,fa.Designation AS famille, a.CB , a.Designation  AS article,sum(df.qte) AS qte, sum(df.ttc) AS ttc ,
		...
		while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){		
								$key_cin = $row['codeVendeur'];
								
							
								if (!isset($vendeur[$key_cin])) {
									$vendeur[$key_cin] = array();
									$vendeur[$key_cin]['codeVendeur']=$row['codeVendeur'];
									$vendeur[$key_cin]['Nom']=$row['Nom'];
									
								} 
								$i=$i+1;
										$key_famille=$row['IdGamme'];
										if (!isset($vendeur[$key_cin][$key_famille])) {
										$vendeur[$key_cin][$key_famille]['IdGamme']= $row['IdGamme'];
										$vendeur[$key_cin][$key_famille]['gamme']= $row['gamme'];							
										}
										$keyArticle= $row['Reference'];
										if (!isset($vendeur[$key_cin][$key_famille][$keyArticle])) {
										$vendeur[$key_cin][$key_famille][$keyArticle]['CB']= $row['CB'];
										$vendeur[$key_cin][$key_famille][$keyArticle]['Ref']= $row['Reference'];	
										$vendeur[$key_cin][$key_famille][$keyArticle]['article']= $row['article'];
										$i=0;
										}
											if($vendeur[$key_cin][$key_famille][$keyArticle]!=""){
												$vendeur[$key_cin][$key_famille][$keyArticle][$i]['UniteVente']= $row['UniteVente'];
												$vendeur[$key_cin][$key_famille][$keyArticle][$i]['qte']= $row['qte'];
												$vendeur[$key_cin][$key_famille][$keyArticle][$i]['ttc']= $row['ttc'];
											}
		
		}
	//echo "<pre>";	print_r($vendeur);echo "</pre>";
	$sum_famille_qte=0;$sum_famille_ttc=0;$sum_vnd_qte=0;$sum_vnd_ttc=0;
		foreach($vendeur as $u=>$vnd){
		
			if($i%2 == 0) $c = "pair";
			else $c="impair";
			?>
			<tr class="pair" style="height:50px;font-size:14px;">
				<td align="<?php $_SESSION['align'] ; ?>" > <strong><?php echo $vnd['codeVendeur']; ?> </td>
				<td align="<?php $_SESSION['align'] ; ?>" colspan="7" > <strong> <?php echo $vnd['Nom'];?> </strong></td>
			</tr>
					<?php foreach($vnd as $famille){
						if(is_array($famille)){ ?>
					<tr  class="impair">
						<td colspan="2"></td>	
						<!--td align="<?php $_SESSION['align'] ; ?>"> <strong><?php echo $famille['IdGamme']; ?></strong></td-->
						<td align="<?php $_SESSION['align'] ; ?>" colspan="6" > <strong><?php echo $famille['gamme']; ?></strong></td>						
					</tr>
				
					    <?php foreach($famille as $article){	
						      if(is_array($article)){ ?>
							<tr style="background-color:#f0f9ff">	
								<td colspan="3"></td>
								<td><?php echo $article['Ref']; ?></td>
								<td  colspan="4"><?php echo $article['article']; ?></td>
							</tr>
						...
						$sum_vnd_qte+=intval($Unite['qte']);
						$sum_vnd_ttc+=floatval($Unite['ttc']);						
							}}	  
						...
						<td colspan="4"></td>
						<td align="right" colspan="2"><strong><?php echo $trad['label']['total']; ?>  <?php echo $famille['gamme']; ?> : </strong></td>
						<td style="direction: ltr;"  align="<?php echo ($_SESSION['lang'] == 'ar' ) ? 'left' : 'right'; ?>" ><?php echo number_format($sum_famille_qte,0," "," "); ?></td>	
					...
					<?php $sum_famille_qte=intval(0);$sum_famille_ttc=0;
						...
						<td colspan="5"></td>
						<td align="right" ><strong><?php echo $trad['label']['totalGlobal']; ?> :</strong></td>
						<td style="direction: ltr;" align="<?php echo ($_SESSION['lang'] == 'ar' ) ? 'left' : 'right'; ?>">
						<?php echo number_format($sum_vnd_qte,0," "," "); ?></td>	
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'Vente.php?delPlusieursArticle',clearForm:false});		
						}
					});
				}			
		}	
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['statistic']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;&nbsp;<?php echo $trad['Menu']['venteVendeur']; ?></div>
...
<input name="DATED" type="hidden" value=""/>	
			  ...
			  <input name="button" type="button"  onClick="rechercher();" value="<?php echo $trad['button']['rechercher']; ?>" class="bouton32" action="rech"			  title="<?php echo $trad['button']['rechercher']; ?> " />
				...
				<input class="formTop"  name="CodeVnd" id="CodeVnd" type="text" size="25" />	
				</div>
					</td>
...
$(document).ready(function(){	
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'Vente.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'Vente.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='Vente.php?add';	
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='Vente.php?mod&ID='+id;
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
