			...
			//---------------------------select qteChargementGlobal--------------------------------//
				...
				if( ($error=="")) {
					...
					<table border="0" width="100%">
							<tr><td width="150" align="right" valign="top">La quantité disponible :</td><td valign="top"><strong><?php echo 	$QteDisp; ?></strong></td>
							<tr><td align="left" valign="top">Unité :</td><td valign="top"><strong><?php 	echo $_GET['Unite']; ?></strong></td></tr>
							<td align="left" valign="top">Colisage:</td><td valign="top"><strong><?php 	echo  substr($Colisage, 1); ?></strong></td>
						</tr>
					
					<script type="text/javascript"> 					
						$('#InfoArticle').show('slow');						
					</script>					
			<?php
...
<?php
	if(isset($_SESSION['lignes']) && count($_SESSION['lignes']) != 0){
        ...
        <td  colspan="2">
			...
			<form id="formLigne" action="" method="post" name="<?php echo $ligne; ?>" >
				...
				<td width="110"><?php echo $row['Unite']; ?></td>
				
				<input type="hidden" value="<?php echo $row['IdArticle']; ?>" name="IdArticle[]">
				<?php 
				if((floatval($row['Qte']*$row['Colisage'])) < (floatval($row['QteDisp']) )) $etat="negatif";else $etat="positif";
					?>
				<td width="110"  align="right">
					<input id="QteCh" action="QteCh" size="7" col="<?php if( $row['Colisage']=="0") echo "1" ; else echo $row['Colisage']; ?>" 
					onblur="VerifQte('<?php echo $k;?>',<?php echo floatval($row['QteDisp']); ?>,<?php echo intval($row['Colisage']); ?>)" 
					class="QteCh<?php echo $k;?>" value="<?php echo $row['Qte']; ?>"
					STYLE=";TEXT-align:right;" onkeypress="return isDecimal(event,this)"    class="<?php echo $etat;?>" 
				name="QteCh[]" type="text"  ></td>
				
					<td  align="right" style="">
				<input id="Colisage" action="Colisage" size="3"  class="" value="<?php echo $row['Colisage']; ?>" 						
				name="Colisage[]" type="hidden"   >
				<?php echo $row['Colisage']; ?>
					</td>
					
				<td width="110"  align="right">
				<input type="text" name="QteDmdParPiece"  size="7" Id="QteDmdParPiece<?php echo $k;?>" value="<?php echo floatval($row['QteDmdParPiece']); ?>" readonly>
				</td>
			
			
				<td width="110"  align="right"  class="positif"> 
					<input type="text" name="QteDisp"  size="7" Id="QteDisp<?php echo $k;?>" value="<?php echo floatval($row['QteDisp']); ?>" readonly>
	...
	?>
		<script language="javascript">
				function VerifQte(id,QteDisp,colisage){
			
				//	QteChar=parseFloat($("#QteCh"+id).val());
						QteChar=0;
				//	if((colisage!="") && (colisage!="0") ){QteChar =QteChar*colisage;}  
			
					// qte du table html
					 var QteHtml=0;var QteCh=0;
					 QteDispo=$('table#table1 tr:last input[name=QteDisp]').val();
					// alert(QteDispo);
					// var QteTotlaDisp=QteDisp;
					 //	alert("QteTotlaDisp "+QteTotlaDisp);
				...
				// QteCh=parseFloat($("#QteCh"+id).val())+parseFloat($(this).val());
				//alert("QteCh "+QteCh);
				
					/*$("input[name=QteDmdParPiece]").each(function(index) {
			
					   QteHtml=parseFloat(QteHtml)+parseFloat($(this).val());

					});
					 
				
					 QteTotlaDisp=QteDisp-QteHtml-parseFloat($("#QteDmdParPiece"+id).val());*/
						//alert("QteDisp "+QteDisp+" QteHtml "+QteHtml+" qtedemde "+parseFloat($("#QteDmdParPiece"+id).val()));
						//	alert("QteCh "+QteCh);
						if(QteCh > QteDispo){
						alert('La qantité chargée '+QteCh+' ne doit pas dépasser la quantité du stock : '+QteDispo);
						//alert('La qantité chargée  ne doit pas dépasser la quantité du stock :');
						...
						$('table#table1 tr:last input[name=QteDisp]').val(QteDispo);
						
						//$("#QteDisp"+id).val(QteTotlaDisp);
					}
				}
...
$sql = "
		SELECT 
			...
			qte as Qte,Unite,d.idArticle as IdArticle,d.colisage,
			a.designation as NomArt,a.CB as CodeaBarre
		...
		$Colisage=1;
			...
			//---------------------------select qteChargementGlobal--------------------------------//
				$sql2 = "SELECT isnull(sum(CASE WHEN dc.reste =0 THEN dc.qte ELSE dc.reste END ),0) as qte
						...
						dc.IdArticle=? and idColisage =? 	group by dc.IdArticle,idColisage";
				$stmt2 = sqlsrv_query( $conn, $sql2, $params1 );	
				if( $stmt2 === false ) {
					$errors = sqlsrv_errors();
				...
				sqlsrv_fetch($stmt2) ;
				$qteChargementGlobal = sqlsrv_get_field( $stmt2, 0);
							...
							$ligneArray["QteDisp"]=$qteEntreeGlobal-($qteChargementGlobal*$row["colisage"]);
	   ...
	   if( (isset($_SESSION['lignes'])) && (count($_SESSION['lignes'])!=0))  {
		$t=0;	
			...
			//---------------------------select qteChargementGlobal--------------------------------//
				...
				// la qte demandé d'un article dans la table session
				$QteDispSession=0;
				/*foreach($_SESSION['lignes'] as $ligne=>$contenu){
					if($contenu["IdArticle"]==$tabArt[0])
					{
						if($contenu["Colisage"]==0)
						$QteDispSession=$QteDispSession+$contenu["Qte"];
					else $QteDispSession=$QteDispSession+$contenu["Qte"]*$contenu["Colisage"];
					}
				}*/
				/*ECHO floatval(end($_SESSION['lignes']));return;;*/
			
				$QteBeforeLaste=array_slice($_SESSION['lignes'], -1, 1);
		
				/*echo  ($QteBeforeLaste[0]["QteDmdParPiece"]);
				return;
				*/
				
				$r=end($_SESSION['lignes']);
				$QteLaste=end($_SESSION['lignes']);
	
			//	echo floatval($QteLaste['QteDisp'])."       ".$QteBeforeLaste[0]["QteDmdParPiece"];RETURN;
			//if( count($QteBeforeLaste==0
				$qteGeneral= floatval($QteLaste['QteDisp'])-$QteBeforeLaste[0]["QteDmdParPiece"] ;
				
			//echo $QteDispSession;
			//$qteGeneral=$LastligneSession['QteDisp']; 
			 ...
			 foreach($_SESSION['lignes'] as $ligne=>$contenu){
				...
				/*	else if (   (floatval($_POST['Qte']) * floatval($Col)) > ($qteGeneral)) {
					
							?>
						<script language="javascript" type="text/javascript">
							alert('Attention,la quantité chargée <?php echo $_POST['Qte']." * ".floatval($Col);?>  doit être inférieur à la quantité du stock.<?php echo $qteGeneral;?> ');
							//	alert('Attention,la quantité chargée <?php echo $_POST['Qte']." * ".floatval($Col);?>  doit être inférieur à la quantité du stock.<?php echo $qteGeneral;?> ');
						</script>
						<?php
						$t=0;break;	
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 
		...
		<?php 
				$sql = "select IdArticle, Designation,CB as CodeBarre,Unite from articles ";
  	            $reponse=sqlsrv_query( $conn, $sql, array(), array( "Scrollable" => 'static' ) );	
				$option="";				
				while ($donnees =  sqlsrv_fetch_array($reponse))
				{
	   ...
	   filter: true,placeholder:'S&eacute;lectionnez l\'article ',single:true,maxHeight: 200,
	     onClick: function(view) {
			...
			$("#Unite").val(res[2]);
				  ...
				  $("#InfoArticle").load('chargements.php?InfoArticle&IdArticle='+res[0]+'&Unite='+res[2]);
            }
...
else
{
	...
	<table width="100%" border="0">
      <tr class="entete">
        <td width="30%">Numéro chargement </td>
        <td width=""> Vendeur  </td>
		...
		while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){
			...
			<tr  class="<?php echo $c; ?>">
				...
				<td>    <input type="button" style="display:none" title="Imprimer "   value="" class="Imprimer tool" onClick="Imprimer('<?php echo $row['idFiche']; ?>');" />
				  <input type="button" title="Détails de chargement"   value="détails" class=" tool"
