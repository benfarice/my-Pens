...
//parcourir($_POST);return;
		...
		$target_path="";
		/* --------------------Begin transaction---------------------- */
		if ( mysql_begin_transaction( $conn ) === false ) {
			$error="Erreur : ".mysql_errors() . " <br/> ";
		}
	  	...
	  	if(isset($_FILES['file']))
			...
			$target_path = "imageMagasin/" . $nameFile;     // Set the target path with a new name of image.
		...
		$reqInser1 = "INSERT INTO ".$tableInser." (nom ,prenom ,intitule ,adresse ,departement,ville,patente,IdFiscale,formeJ,rc,
						longitude,latitude,idVendeur,idDepot,idTypeClient,CodeClient,ImgMagasin
		...
		$stmt1 = mysql_query( $conn, $reqInser1 );
		if( $stmt1 === false ) {
		
			$error.="Erreur : ".mysql_error($conn) . " <br/> ";
		}
...
if(isset($_GET['chargerSecteur'])){
	...
	$sql = "SELECT d.iddepartment,d.codeDepartement,d.Designation FROM departements d where idVille=".$_GET['IdVille'];
			...
			$reponse=mysql_query( $conn, $sql);         
			/*   if( $reponse === false ) {
				 die( print_r( mysql_errors(), true));
			}*/
			
		$nRes = mysql_num_rows($reponse);
...
<div id="tabs">
  ...
  <div id="Clt">
		...
		<td>
            ...
            <td colspan="">
			<textarea rows="2" cols="25" name="Adresse"  id="Adresse"></textarea>
            ...
            <td valign="top">
               <input class="file" type="file" name="file" id="file" size="25"
			   tabindex="1"   value="" />
			 </td>
