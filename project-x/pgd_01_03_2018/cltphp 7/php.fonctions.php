...
function Increment_Chaine_F($chp,$table,$orderBy,$conn,$req){
	$errors="";$error="";
		if($req==""){
		$sql="SELECT  (".$chp.") FROM ".$table." ORDER BY ".$orderBy." DESC LIMIT 1";
		$params = array();
		}
		else {
			$sql="SELECT  1 (".$chp.") FROM ".$table." where ".$req." ORDER BY ".$orderBy." DESC LIMIT 1";
			
		}

		$resAff=mysqli_query( $conn, $sql);  
	...
	if( $resAff== false ) {
	 ...
	 if(mysqli_num_rows($resAff) !=0){	

		$row = mysqli_fetch_array( $resAff);
//		print_r($row);
		$LastNum = $row[$chp];		
