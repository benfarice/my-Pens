...
$error="";
	...
	$paramsUpd= array($_GET['idCltGrp'],$_GET['Type']) ;	
	$reqUpdate="update ".$tableInser." set etat=0 where idTypeClient  =(?) and type like ? ";
	...
	$paramsUpdF= array($_GET['ID'],$_GET['Type']) ;	
	$reqUpdateF="update ".$tableInser." set etat=1 where IdFiche  =(?)  and type like ? ";
...
$paramsVerif= array($IdCltGrp,$TypeCltGrp) ;

//-----------------Verifier si le clt ou grp ont deja une fiche de tarif----------------//
$reqVerf="select idFiche from ".$tableInser." where etat=1 and IdTypeclient =? and type like ? " ;
...
if($row_count!=0){
	...
	$paramsUpd= array($IdCltGrp,$TypeCltGrp) ;	
	$reqUpdate="update ".$tableInser." set etat=0 where idTypeClient  = ? and type like ? " ;
...
$NumFiche= "NT".Increment_Chaine_F("numFiche",$tableInser,"idFiche");

$reqInser1 = "INSERT INTO ".$tableInser." ([numFiche] ,[idTypeClient] ,[date]  ,[operateur] ,[idDepot] ,[type],[etat]) values 	(?,?,?,?,?,?,?)";
$params1= array($NumFiche,$IdCltGrp,$DateFiche,$UserId,$IdDepot,$TypeCltGrp,1) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 
			<table border="0" cellpadding="5" cellspacing="5">
			<tr>
					<td  valign="top" ><div class="etiqForm"  style="text-align:left; " 
			id="DATE_PIECE" > <strong>Article</strong> : </div>
            </td>
			</tr>
			<tr>
			<td>
		<div id="grpCaisse" style=" width:240px; float:left;"><select style="width:240px; vertical-align:top"
		multiple="multiple" name="ListeArt" id="ListeArt"  tabindex="3" class="ListeArt" >
...
unset($_SESSION['lignes']);
	...
	$sqlA = "
		SELECT 
			t.numFiche,idFiche,t.idTypeClient,etat,date as dateFiche,type,nom,prenom,Designation,
			c.idTypeClient
		FROM 
			$tableInser t
			left join typeClients  tc on tc.IdType= t.IdTypeclient
	...
	<?php
		...
		if($row['etat']=='1') { $cl="act";}
			...
			<td align="left"  > <?php echo $row['numFiche'];?></td>
				...
				<td>    
				  <input type="button" title="Détails tarif"   value="détails" class=" tool" onClick="Imprimer('<?php echo $row['idFiche']; ?>');" />
				  </td>
...
function ActiverM(id,idCltGrp,Type){
	jConfirm('Confirmer l\'opération ?', null, function(r) {
		if(r)	{
		var url='details_tarifs.php?Act&ID='+id+'&idCltGrp='+idCltGrp+'&Type='+Type;
		$('#resG').html('').load(url);
							}
	});
}
