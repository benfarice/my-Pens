 ...
 foreach($_SESSION['lignes'] as $ligne=>$contenu)
{
	...
	$reqInser2 = "INSERT INTO  detailmouvements([idArticle],[qte],[pa],[idDepot],idMouvement ) values (?,?,?,?,?)";
...
if(isset($_GET['chargerAllArticle'])){
			...
			$sql = " select a.IdArticle, a.Designation,a.Reference as Reference,a.Unite from articles a
		    ...
		    if($nRes != 0)
		     while ($donnees =  sqlsrv_fetch_array($reponse))
            {
$Options.="<option value=".$donnees['IdArticle'].",".$donnees['Reference'].",".$donnees['Unite'].",TESTLLLLLLLLLLL AMINA>".strtoupper($donnees['Reference'])."</option>";
			}
 ...
 $('#ListeArt').multipleSelect({
	   filter: true,placeholder:'<?php echo $trad['label']['SelectArticle'];?> ',single:true,maxHeight: 200,
	     onClick: function(view) {
			...
			$("#UniteArt").val(res[2]);
				  $("#resart").load('entrees.php?InfoArticle&IdArticle='+res[0]+'&Unite='+res[2]);
            }
		...
		$('#SFamille').multipleSelect({
		  filter: true,placeholder:'<?php echo $trad['label']['SelectSousFam'];?> ',single:true,maxHeight: 100,
		      onClick: function(view) {
				...
				$('#gammes').load("entrees.php?chargerGamme&IdSSFamille="+view.value);
...
$('#Famille').multipleSelect({
	   filter: true,placeholder:'<?php echo $trad['label']['selectFamille'];?> ',single:true,maxHeight: 100,
	    onClick: function(view) {
				if(view.checked = 'checked')
				$('#ssfamilles').load("entrees.php?chargerSSFamille&IdFamille="+view.value);
			   //  alert(view.label + '(' + view.value + ') ' + (view.checked ? 'checked' : 'unchecked'));
...
$(document).ready(function(){
		...
		$('#grpCaisse').load("entrees.php?chargerAllArticle");
 $(".show").click(function(){
		  url="entrees.php?searchArticle";
...
function suppression(idPos){
			...
			jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							$('#act').attr('value','supp'); 
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'entrees.php?suppression&idPos='+idPos,clearForm:false});		
						}
					})
...
function modification(nomPromo){
		...
		var url='entrees.php?modification&nompromo='+nomPromo ;
	...
	function filtrer(){
		...
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'entrees.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){

		$('#formRechF').ajaxSubmit({target:'#formRes',url:'entrees.php?rech'})
...
$(document).ready(function(){
	...
	/*$("label[data-group=group_0]").parent(".group").css("border","1px solid red");
		...
		$('#formRes').html('<center><br/><br/><img src="images/loading.gif" /></center>').load('entrees.php?aff');
				$('#box').dialog({
					autoOpen		:	false,
					width			:	1200,
					height			:	540,
    ...
    //hide close button.
					...
					buttons			:	{
						"<?php echo $trad['button']['Fermer'];?>"		: function(){
							$('#resG').load('entrees.php?VideSession');
							$(this).dialog('close');
						},
...
/*******************************************************************/
		...
		if((test==true) && (t1==true)){
			...
			var NomArt= $("#ListeArt").multipleSelect("getSelects", "text");
											...
											if(act == 'modif'){	
													...
													$('#formAdd').ajaxSubmit({
														target			:	'#listBox',
														url				:	'entrees.php?goAddLigne&NomArt='+NomArt,
														method			:	'post',
															success:function(){
																/*$("div[id=ligne]").remove();*/
																$("input[id=Qte]").val("");
	...
	function supLigne(ligne){
			...
			var adr 	= 'entrees.php?supLigne='+ligne;
