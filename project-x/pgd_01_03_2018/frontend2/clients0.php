...
<?php 	foreach($groups as $u=>$v){	?>
		  <div class=" col-md-offset-1 col-md-4    col-md-offset-1   col-sm-12 hvr-grow"   id="cadreType<?php  echo $v['IdType'];?>" 
...
function AfficheCSP(IdType,Dsg){
		...
		$('#TypeClt').val(Dsg);
		$('#BoxData').modal('hide');
		//$('#BoxMap').load("clients0.php?GetCsb&IdType="+IdType);	
	}
$(document).ready(function(){	
$('#BoxData').modal('show');
if(	$('#IdTypeClt').val() !=""){
...
<script language="javascript" type="text/javascript">
  ...
  function AfficheTypeClt(idActv,Dsg){
	//	$('#BoxMap').load("clients0.php?GetTypeClt2");
		...
		if(status == google.maps.GeocoderStatus.OK) {		
		  ...
		  if(results[1]) {
			...
			for (ac = 0; ac < arrDetailAdr.length; ac++) {			
				...
				var lat=res[0];
				var longu= res[1];
				var url="clients0.php?add&Ville="+ville+"&Adresse="+encodeURIComponent(adressComplet)+"&Secteur="+encodeURIComponent(Secteur)+"&long="+encodeURIComponent(longu)+"&lat="+encodeURIComponent(lat);
			
				 $.get(url, null, function(data) {
					//$("#Box").find('.modal-body').html(data).modal('show');
						$("#Box").find('.modal-body').html(data);
			});
			//$('#boxClient').load("clients0.php?add&Ville="+ville+"&Adresse="+encodeURIComponent(adressComplet)+"&Secteur="+encodeURIComponent(Secteur)+"&long="+encodeURIComponent(longu)+"&lat="+encodeURIComponent(lat)).dialog('open');
			}
		}
	});
	}
	...
	//echo toDateSql($_POST['DateF']);return;
		...
		if(isset($_POST['DateD']) && isset($_POST['DateF'])  )
		{
			...
			{ 
				 ...
				 $where.= " where convert(date,date_create) = convert(date, '".($_POST['DateD'])."')";
			}
			else
			{
				 $where.= " where cast(date_create AS date)  between  convert(date, '".($_POST['DateD'])."' ) and convert(date,  '".($_POST['DateF'])."') ";
			}
		}
		else
		{
			$where=" where cast(date_create AS date)='".(date('d/m/Y'))."'";
...
/*execSQL($sql);*/
	...
	/*	$resAff = mysql_query($reqAff)or die(mysql_error());*/
		if($nRes==0)
		{ ?>
					<div class="resAff"  style="text-align:center;min-height:200px;">
						<br><br>
						Aucun r&eacute;sultat &agrave; afficher.
					</div>
					<?php
...
<div class="col-md-12 col-sm-12">
	 <input id="pac-input" class="controlMap"  type="text" placeholder="Recherche">
	<div id="map" >
	</div>
</div>
<script language="javascript" type="text/javascript">
 ...
 <div class="form-group row">
  ...
  <div class="row">
   <div class="col-md-7 col-sm-9">
				<label class="col-md-4 col-sm-12 control-label ">Localisation :</label>
			...
			<div class="  col-md-3 col-sm-12 form-group">
				&nbsp;<input class="btn btn-primary" type="button" name="Localisation"  value="Localiser" id="Localisation"
				tabindex="1" /> 
			</div>
	   
   </div>
      
		<div class=" col-md-5 col-sm-12 row form-group  ">  
   ...
   <div class="col-md-5 col-sm-12  ">
			...
			<input type="button" value="Activité client" onclick="getActivite('open')" class="btnCmdArt btn btn-primary"></div>
	...
	<div class=" col-md-offset-1 col-md-5 col-sm-12 ">  
				...
				<input type="button" value="CSP client" onclick="getCSB_Clt('open')"
  ...
  <div class="form-group row">
	   <div class="col-md-5 col-sm-12">
				<div class="col-md-4 col-sm-4 form-group"> 
				 <input type="button" value="Type client" onclick="getType('open')"
									class="btnCmdArt btn btn-primary">
				</div>
...
<div class="form-group row">
   <div class="col-md-5 col-sm-12">
			...
			<label class="col-md-4 control-label ">Id fiscale :</label>
		...
		<div class=" col-md-offset-1 col-md-5 col-sm-12 ">  
					...
					<label class="col-md-4 control-label ">Forme juridique :</label>
  ...
  <div class="form-group row">
   <div class="col-md-5 col-sm-12">
			...
			<label class="col-md-4 control-label ">Superficie (m²):</label>
			 ...
			 jConfirm('Voulez-vous vraiment terminer la saisie?', null, function(r) {
					if(r)	{
						if(act == 'mod'){	
												$('#formMod').ajaxSubmit({
														target			:	'#resMod',
														url				:	'clients0.php?goMod',
														method			:	'post'
													}); 
												
											}else{
												//$('#progress-div').show();
												ajaxindicatorstart('Opération en cours, merci de patienter..');
												$('#formAdd').ajaxSubmit({
														target			:	'#resAdd',
...
$('#Ville').multipleSelect({
		 ...
		 width:'100%',
		      onClick: function(view) {
				...
				$('#Secteurs').load("clients0.php?chargerSecteur&IdVille="+view.value);
  ...
  var Villes = <?php echo json_encode($TabVilles ); ?>;
        for (var i = 0; i < Villes.length; i++) {
			...
			//alert(idVille);	
				...
				$('#Secteurs').load("clients0.php?chargerSecteur&IdVille="+idVille+"&Secteur="+encodeURIComponent("<?php echo $_GET['Secteur']; ?>"));
			}
        }	
});
//$('#Secteurs').load("clients0.php?chargerSecteur&IdVille=<?php  echo $IdVille ;?>&Secteur="+encodeURIComponent("<?php echo $_GET['Secteur']; ?>"));
...
$('#Localisation').click(function(){
	//	$('#BoxMap').html('<center><br/><br/>Merci de patienter pendant le chargement... <br/><img src="../images/loading2.gif" /></center>').load('clients0.php?getMap').dialog('open');
	 var $modal = $('#BoxData');
		var url='clients0.php?getMap';
    $.get(url, null, function(data) {
...
.ui-widget-header {
 ...
 <div   class="row col-md-12 headVente " id="infosGPS" ><a href="index.php"><img src= "../images/home.png"  /></a>
		&nbsp; <span >Gestion des clients</span> &nbsp;</div>
...
<div  class="row row-centered ">
 ...
 <form id="formRechF" method="post" name="formRechF" > 
			...
			<div class="middleLabel "  ><label for="user_lastname">Date : &nbsp;</label> </div>	 
			</div>
			...
			<div  class="  col-md-1 col-sm-12  centerLabel" > <label for="user_lastname">à</label>	
			</div>
			<div  class=" middleLabel   col-md-3 col-sm-12 form-group " >		
					<input  g="date" id="DateF" tabindex="2" name="DateF" type="text"  class="form-control"  size="10" maxlength="10" onChange="verifier_date(this);" value="<?php echo date('d/m/Y'); ?>"/>	
					<input name="DATED" type="hidden" value=""/>	
			</div>
		
			<div  class="  col-md-4 col-sm-12  centerLabel  ">	
			
			&nbsp;<input type="button" value="Rechercher" class="btn btn-primary"  id="rech" action="rech" 
			onclick="rechercher()"; />
			
			&nbsp;<input type="reset" value="Effacer" class="btn btn-primary"  id="reset" action="effacer" 
			 />
			<input type="button" value="Ajouter" class="btn btn-primary"   action="ajout" 
			 onclick="ajouter()"
			 />
...
<div id="boxActiv" >
 	 ...
 	 <div  style="display: none; padding-left: 0;"   data-backdrop="static" class="modal fade" data-keyboard="false" id="Box" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"  
	 aria-hidden="false">
           <div class="modal-dialog">
             <div class="modal-content">
                  <div class="modal-header">                   
                    <h4 class="modal-title" id="myModalLabel">Gestion des clients</h4>
                  </div>
          ...
          <img src="http://conferoapp.com/icons/preloader.gif" class="progress">
				  ...
				  <input type="submit" value="Terminer" id="Terminer"   class="btn btn-primary" onclick="AjoutClt()" name="save" />
				<Input type="button" class="btn btn-primary" onclick="CloseBox('Box')"  value="Fermer" />
...
/*$('#box').dialog({
		...
		// $('#formRes').load('clients0.php?add');
		$('#formRes').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load('clients0.php?aff');
...
function rechercher(){
	...
	$('#formRes').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>');
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'clients0.php?rech'});
		}
...
function closeBoxClient(){
 ...
 function ajouter(){
		$('#act').attr('value','add');	
		/*var url='clients0.php?getLocation';		
...
function getActivite(){
  var $modal = $('#BoxData');
		var url='clients0.php?GetAct';
    $.get(url, null, function(data) {
      $modal.find('.modal-body').html(data);
    })
	
		//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("clients0.php?GetAct").dialog('open');
	}
function getType(open){

var $modal = $('#BoxData');
		var url='clients0.php?GetTypeClt2';
...
function getCSB_Clt(open){
	var $modal = $('#BoxData');
		var url='clients0.php?GetCsb';
	if(open!='undefined ') {
		//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("clients0.php?GetCsb").dialog('open');
		$.get(url, null, function(data) {
		  $modal.find('.modal-body').html(data);
		})
	}else 
...
//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("clients0.php?GetCsb");
	}
