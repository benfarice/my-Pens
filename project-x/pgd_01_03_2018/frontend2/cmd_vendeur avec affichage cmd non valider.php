								...
								/*	  $Options.="<option value=".$row['IdType'].">". stripslashes (htmlentities($row['IdType']."  ".
										...
										if (!isset($groups[$key])) {
											...
											$groups[$key]['DsgFamille']=$row['DsgFamille'];
												...
												//	$groups[$key][$keySousFam][$keyMarque][$keyGamme][$i]['IdDtlCmdVdr'] = $row['IdDtlCmdVdr'];							
...
?>
<DIV>
<div style="width:1081px; display:flex;align-items:center; padding:2px 0;border-right:none;" 
 class="headVente heaLeft">
							<a href="index.php"><img src="../images/home.png" height="64" width="64" 
							class=""> </a>
						<div >&nbsp;> <span  Class="TitleHead" onclick="">Commande dépôt</span></div> 
</div>
<div style="width:195px;" class="headRight" ><input type="button" value="Ajout article" class="btnCmdVendeur hvr-grow"  onclick="AffCatalogue();"/></div>
</div>
<DIV class="haut">
...
$sqlCmdVdr = "
				SELECT 
			d.idDetailCommandeVendeur IdDtlCmdVdr,d.qte as Qte,a.Unite,d.idArticle as IdArticle,d.colisage Colisage,c.idCommandeVendeur,
			a.designation as NomArt,a.Reference as CodeaBarre,t.pvHT PV ,a.Reference RefArticle,a.TVA,
			fa.idFamille IdFamille,fa.Designation,
			fa.codeFamille RefFam,sf.codeSousFamille RefSousFam,
			g.Reference RefGamme,
			g.Designation Gamme,
			...
			detailcommandeVendeurs d 
				...
				INNER JOIN Familles fa ON sf.idFamille=fa.idFamille 
			 where f.idTypeClient=1 and f.type like 'Groupe' and  f.etat=1						
			and c.idDepot=".$_SESSION['IdDepot']."  and idVendeur = ? and c.etat=0";
		$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
		 $paramsC = array($_SESSION['IdVendeur']);
		 $resAffC = sqlsrv_query($conn,$sqlCmdVdr,$paramsC,$options ) or die( print_r( sqlsrv_errors(), true));
		 $nResC = sqlsrv_num_rows($resAffC);
		 //ECHO $sqlCmdVdr."<br>"; parcourir($paramsC); 
		 if( $nResC!=0){
			 $IndexLigne=0;
			 while ($row =  sqlsrv_fetch_array($resAffC))
				{
							$ligneArray["IdDtlCmdVdr"]=$row["IdDtlCmdVdr"];
							$ligneArray["IdLigne"]=$IndexLigne;
							$ligneArray["IdArticle"]=$row["IdArticle"];
							$ligneArray["NomArt"]=$row["NomArt"];
							$ligneArray["Qte"]=$row["Qte"];
							$ligneArray["PV"]=$row["PV"];
							$ligneArray["Gamme"]=$row["Gamme"];
							$ligneArray["TVA"]=$row["TVA"];
							$ligneArray["HT"]=$row["Qte"]*$row["Colisage"]*$row["PV"];										
							$ligneArray["TTC"]=($row["Qte"]*$row["Colisage"]*$row["PV"]) +$row["TVA"];
...
$idFiche="";
  ...
  // verifier si l'article est déja commandé mais
  if( $contenu['IdDtlCmdVdr'] !="") {
	/* verifier si  la qte de l'article déja commandé est différente à laqte du bdd l le supprime et l'ajout dans la nouvelle cmd 
		si le vendeur a modifié la qte cmd en avant*/
		$sql2 = "SELECT qte ,colisage 
						FROM detailCommandeVendeurs dc  
						left join commandeVendeurs cv on cv.idCommandeVendeur=dc.idCommandeVendeur
						WHERE
						dc.idDetailCommandeVendeur=?  and cv.etat=0	 	and dc.idDepot=".$_SESSION['IdDepot']." 
						and cv.idVendeur = ".$_SESSION['IdVendeur']." ";
						$params1= array($contenu['IdDtlCmdVdr']) ;
				$stmt2 = sqlsrv_query( $conn, $sql2, $params1,array( "Scrollable" => SQLSRV_CURSOR_KEYSET ) );	
			//echo $sql2."___".$tabArtilce[0]['IdArticle'];
				if( $stmt2 === false ) {
					$errors = sqlsrv_errors();
					echo "Erreur : get qte commande ".$errors[0]['message'] . " <br/> ";
					return;
				}else 
				{
					...
					if ($nbr!=0){
						sqlsrv_fetch($stmt2) ;
						$qteDejaCmd = sqlsrv_get_field( $stmt2, 0);
						$colDejaCmd = sqlsrv_get_field( $stmt2, 1);
						echo "qte cmdé: $qteDejaCmd  colDejaCmd:$colDejaCmd"; 
						if(($qteDejaCmd!=$contenu['Qte']) && ($colDejaCmd ==$contenu['Colisage'])){
							//ajout article dans la nouvelle cmd et le supprime dans l'ancien cmd
						}else if(($qteDejaCmd==$contenu['Qte']) && ($colDejaCmd !=$contenu['Colisage'])){
							////ajout article dans la nouvelle cmd
						}						
						return;
						}
				}
...
if( $test==false)  $error="Erreur d'envoi de la facture";
if($error=="" ) {
    sqlsrv_commit( $conn );
	 unset($_SESSION['lignesCmd']);
     ?>
