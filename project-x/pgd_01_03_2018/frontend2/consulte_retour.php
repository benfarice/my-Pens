...
<input type="hidden" value="<?php  echo $row['IdArticle']; ?>" name="idArticle[<?php echo $i; ?>]">
	<div  class="<?php echo $c; ?>" >
			...
			<div  align="right" class="divArticleLigne" style="width:380px" >	
				<input class="numberOnly" type="text" disabled  style="width:100px" value="<?php  echo $QteRetour[0] ;?>" 
				size="5" name="qtRetourBoxs[]" onkeypress="return isEntier(event) " />&nbsp;<?php echo $trad['label']['Boxs'];?>
				
					<input class="numberOnly"  disabled type="text" style="width:100px" value="<?php  echo $QteRetour[1] ;?>" 
...
<div class="btnV" style="margin:10px 10px 0 0">
	<input type="button" value="<?php echo $trad['button']['Enregistrer'];?>" class="btn chpinvisible" onclick="Valider(<?php echo $idCmd; ?>,<?php echo $IdAvoir; ?>)"/>
			...
			if(test==true) {		
			 jConfirm('<?php echo $trad['msg']['terminerOperation'];?>', "<?php echo $trad['titre']['Alert'];?>", function(r) {
					if(r)	{
							$('#formAdd').ajaxSubmit({
									target			:	'#result',
									url				:	'consulte_retour.php?goAddRetour&idFacture='+IdCmd+'&&IdAvoir='+IdAvoir,
									method			:	'post'
							}); 
							return false;
			           }			
...
function getcommande(numFac){
	var url='consulte_retour.php?getCommande&&numFac='+numFac;	
...
$sql = "
	SELECT a.IdFacture IdFacture,NumFacture numCommande,v.nom+ ' ' + v.prenom AS nom,a.[DateR] as DateR 
				,f.idClient IdClient,f.idVendeur,c.intitule AS clt,EtatAvoir FROM
				avoir_client a 
				right join factures f on f.idFacture=a.idFacture
			INNER JOIN vendeurs v ON f.idVendeur=v.idVendeur	
			INNER join clients c on c.idClient=f.idClient AND f.IdFacture=a.IdFacture
			where v.idDepot=?   and f.idVendeur=? group by 
			 a.IdFacture ,NumFacture ,v.nom,v.prenom ,a.DateR 
				,f.idClient ,f.idVendeur,c.intitule,EtatAvoir 
			order by a.IdFacture desc";
	
	 $params = array($_SESSION['IdDepot'],$_SESSION['IdVendeur']);
...
<DIV class="ListeCmd">
	<div class="enteteL" >
			...
			<div  class="divArticleL" style="width:292px" ><?php echo $trad['label']['Client'];?>  </div>	
...
$k++;
$back="";
if($row['EtatAvoir']==1) $back="backGreenn";
else if($row['EtatAvoir']==0) $back="backYellow";
?>
...
<div style=" display:flex;align-items:center; padding:2px 0;"  class="headVente">
						...
						<div >&nbsp;> <span  Class="TitleHead" onclick=""><?php echo $trad['label']['ConsultAvoir'];?></span></div> 
</div>
...
$(document).ready(function() {
		//$.validator.messages.required = '';
  		$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('consulte_retour.php?aff');		
});
function rechercher(){
		$('#formAdd').ajaxSubmit({target:'#formRes',url:'consulte_retour.php?aff'})
