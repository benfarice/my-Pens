...
$_SESSION['rel_in_var']=0;


if(isset($_GET["VerifPwd"])){
//parcourir( $_POST) ;
//echo "hereee";return;
...
$msg="";
	$sql = "SELECT pwd
			 FROM utilisateurs 
			 where Login like  'admintest'  and pwd like '".$_POST['PwdAdmin']."'";
			echo $sql;
		$params = array();	
	...
	{
		$login=false;
	    $msg=$trad['login']['MsgErrorPwd'] ;//"MsgErrorPwd incorrect. Veuillez réessayer.";
	}
		if($login==true)
		{
			...
			$('#formcon').html('');
			$('#formcon').hide( 1000 );
			window.location.href = 'validation_retour.php';
			</script>
			<?php
		}
		else
...
<a href="inventaire_y.php" class="cadreIndex hvr-grow clot_tournee" >  
		<div  class="childIndex"> 
		<img src="../images/invetory.png" style="width:100px; height:100px" >
...
<a href="view_self_ca.php" class="cadreIndex hvr-grow ">  
		<div  class="childIndex"> 
		<img src="../images/ca.png" style="width:100px; height:100px" >
		<div class="titleIndex1"> <?php echo $trad['label']['ca_vdr'];?></div>
		</div>
...
<a href="consultation_des_factures.php" class="cadreIndex hvr-grow " >
  
		<div  class="childIndex"> 
		<img src="../images/Commande_depot.png" style="width:100px; height:100px" >
			<div class="titleIndex1">consultation des factures</div>
		</div>
...
<div id="boxR" STYLE="text-align:center;background:#fff; padding-top:20px">
		<a  href="retour_fac.php" class="cadreIndex hvr-grow clot_tournee" >  
			...
			<a  href="consulte_retour.php" class="cadreIndex hvr-grow clot_tournee" >  
				<div  class="childIndex"> 
					<img src="../images/precommande.png" style="width:100px; height:100px" >
					<div class="titleIndex1"> <?php echo $trad['label']['ConsultAvoir'];?></div>
				</div>	
		</a>
		<!--a   onclick="OuvrirBoitePwd()" class="cadreIndex hvr-grow clot_tournee" >  
				<div  class="childIndex"> 
					<img src="../images/precommande.png" style="width:100px; height:100px" >
...
<div id="boxVerif" STYLE="text-align:center;background:#fff; padding:20px 40px">
	 <form method="post"  id="FormVerif" NAME="FormVerif">
			<div class="control-group">
		    <label class="control-label" for="password"><?php echo $trad['label']['EnterPWd'] ; ?></label><br><br>
			<div class="controls">
			    <input value=""  name="PwdAdmin" id="PwdAdmin" type="password" style="width:280px;"/>
			</div>
		</div>
		<div class="btnV" style="text-align:center;margin-top:40px">
			  <div id="formcon" ></div>	<br>
		<input type="button" value="<?php echo $trad['button']['Valider'];?>"  class="btn" onclick="VerifierPwd()"/>	
	<input type="button" value="<?php echo $trad['button']['Fermer'];?>"  class="btn" onclick="Fermer('boxVerif')"/>
...
function VerifierPwd(){
	...
	$("#FormVerif").validate({
		...
		var test=$("#FormVerif").valid();						 

		if(test==true){
		  $('#FormVerif').ajaxSubmit({target:'#formcon',url:'index.php?VerifPwd'})
		}else{
...
function OuvrirBoitePwd(){
	$('#boxVerif').dialog('open');
}
function Fermer(box){
	$("#"+box).dialog('close');
}
...
$("#Chg").click(function(){
	//Vente=1 càd c'est vente direct
		$('#res').load("index.php?map&&Vente=1");
	});
$("#ChgPrev").click(function(){
		//Vente=0 càd c'est prevente  
	$('#res').load("index.php?map&&Vente=0");
});
})
