				...
				// controler si  table session contient deja la ligne avec  mm article 
					...
					if(($contenu["IdArticle"]==$_POST["IdArticle"]))
					{
							...
							//	$ligneArray["IdDtlCmdVdr"]=$_POST["IdDtlCmdVdr"];
									...
									$ligneArray["UniteVente"]=$contenu["UniteVente"];
									$ligneArray["NbrBox"]=$contenu["NbrBox"];
									$ligneArray["NbrPalette"]=$contenu["NbrPalette"];
									$ligneArray["NbrColisage"]=$contenu["NbrColisage"];
								$ligneArray["Box"]=$contenu["Box"];
									$ligneArray["Palette"]=$contenu["Palette"];
									$ligneArray["Colisage"]=$contenu["Colisage"];
									
									$ligneArray["UniteVenteClt"]=$_POST["UniteVente"];
...
//$IdClient=4123;
	...
	$sql="select 
		...
		inner join colisages co on co.idArticle=a.idArticle
			...
			INNER JOIN factures fac ON fac.idFacture=df.idFacture
			where 
			 fac.idClient=".$IdClient." 
			AND fac.IdFacture = (
				SELECT  max(fa.idFacture) from  factures fa
					INNER JOIN detailfactures def on def.idFacture=fa.idFacture
								...
								/*	  $Options.="<option value=".$row['IdType'].">". stripslashes (htmlentities($row['IdType']."  ".
									...
									//	else {
												...
												$keyGamme= $row['IdGamme'];
													...
													$groups[$key][$keySousFam][$keyMarque][$keyGamme][$i]['UniteVente'] =$row['UniteVente'];
													...
													$groups[$key][$keySousFam][$keyMarque][$keyGamme][$i]['UniteVenteClt'] ="";
...
<span  class="" ><?php  echo $trad['label']['listeArticle'];?></span-->
<div class="closebtn" onclick="Fermer()" ></div>&nbsp;<?php  echo $trad['label']['listeArticle'];
?>
</div>
<div class="clear"></div>
<?php
					...
					foreach($tabArtilce as $r){//article												
							...
							?>		
								<div class="ligne " >
									...
									<div class="divCmd" Style="width:284px;TEXT-align:center;background:#fff;" >
									<input type="button" value="Commander" onclick="getArticle('<?php  echo $r['IdArticle'];?>')"
									class="btnCmdArt"></div>
								</div>
								<div class="clear"></div>
				<?php 	}}?>									
		  ...
		  <div class="divPVL"> <?php echo $trad['label']['Rupture'];?>  </div>
			...
			foreach($_SESSION['tabArticle'] as $ligne=> $row){
				...
				<div class=" <?php echo $c;?>" onclick="getArticle('<?php  echo $row['IdArticle'];?>','list')">
						...
						$dateDernierVente = $row['dateDernierVente']->format('d/m/Y');
						echo $dateDernierVente;?> </div> 											
						<div class="divColisageL" > <?php if($row['QteStock']!="") {
						echo $row['QteStock']." (";						
						if($row['UniteVenteClt']=="Palette") echo $trad['label']['Palette'];
						if($row['UniteVenteClt']=="Box") echo $trad['label']['Box'];
						if($row['UniteVenteClt']=="Colisage") echo $trad['label']['Colisage'];
						echo ")";
						} ?> </div> 
						<div class="divPVL" > <?php  echo $row['NbrJourRupture'];?> </div>
				</div>
										...
										<div id="resAddArt"></div>
											...
											<TABLE   border="0" width="100%" class="table" cellspacing="2" cellpadding="7">
												<tr><TD width="25%"><?php echo $trad['label']['Ref'];?>:</td><td>	<?php  echo $tabArtilce[0]['Reference'];?>
													...
													$key = array_search($_GET['idArticle'],array_column($tabArtilce, 'IdArticle'));
													$Qte=$tabArtilce[0]["QteStock"];
													if($tabArtilce[0]["UniteVenteClt"]=="") $UniteVente="Box";
													else $UniteVente=$tabArtilce[0]["UniteVenteClt"];
													if($Qte=="") $Qte=1;
													?>
												...
												<td height="140" valign="middle" align="<?php echo $_SESSION["align"];?>">
												...
												data-labelauty="|<?php //  echo $trad['label']['Box']." (".$tabArtilce[0]['Box'].")";?>" aria-label="2"  checked />
											...
											<input type="hidden"
												...
												<tr><TD><?php  echo $trad['label']['SituationActuel'];?>:</td><td Valign="top" align="<?php echo $_SESSION["align"];?>"> 
 ...
 $('input[type="radio"]').click(function(){
   ...
   function CalculPrixArt(IdLigne){
	...
	if(Unite=="Palette"){
		...
		TotalPrice=PriceUnite.replace(" ","") * $("#Qte"+IdLigne).val().replace(" ","");
		//	alert(TotalPrice);
		...
		$(".divPRix"+IdLigne).html("<?php echo  $trad['label']['NbrColisage'] ;?> "+nbrColisage);
...
$reqInser1 = "INSERT INTO [Inventaire] ([IdClient] ,[DateInv]) 	VALUES 	(?,?)";
		//	echo $reqInser1;$_SESSION['IdClt']
$params1= array($_SESSION['IdClt'],$Date) ;
$stmt1 = sqlsrv_query( $conn, $reqInser1, $params1 );

if( $stmt1== false ) {
    $errors = sqlsrv_errors();
    $error.="Erreur : Ajout inventaire ".$errors[0]['message'] . " <br/> ";
}
//---------------------------IdInventaire--------------------------------//
...
$IdInv = sqlsrv_get_field( $stmt2, 0);
 ...
 /*	echo "<p>";
	...
	echo "</p><br>";*/
					if($ligne['Rupture']=='')
					{ 
						$reqInser2 = "INSERT INTO [detailsInventaire] ([IdArticle] ,[QteStock] ,[Colisage] ,[IdInv],UniteVente)
						VALUES (?,?,?,?,?)";
						$params2= array(
						$ligne['IdArticle'],
						$ligne['QteStock'],
...
<a href="qualite_service.php" class="cadreIndex hvr-grow " >
...
$reqInser1 = "INSERT INTO [dbo].[visites]  ([IdTournee] ,[datedebut]  ,[heureDebut]   ,[idClient] ,[idDepot]) 
				values(?,?,?,?,?)";
	$params1= array($_SESSION['IdTournee'],$dateD,$Hour,$IdClt,$_SESSION['IdDepot']) ;
	$stmt1 = sqlsrv_query( $conn, $reqInser1, $params1 );
	if( $stmt1 === false ) {
		$errors = sqlsrv_errors();
...
$sql = "SELECT c.IdClient,c.nom,c.prenom,c.adresse,c.longitude,c.latitude,tc.Designation as Type ,a.DsgActivite ,a.icone  FROM clients c INNER JOIN typeClients tc ON c.idTypeClient=tc.idType INNER JOIN  activites a ON c.IdActivite=a.IdActivite WHERE (c.longitude!='' and c.latitude!='') and c.departement in ( SELECT da.idDepartement FROM affectations a INNER JOIN detailAffectations da ON a.idaffectation=da.idaffectation WHERE a.idVendeur=".$_SESSION['IdVendeur'] . " ) ";
//echo $sql;return;
...
{	
	...
	/*	$client[$i]['longitude']=$row['longitude'];
		...
		$i++;
		$features.="{position: new google.maps.LatLng(".$row['latitude'].",".$row['longitude']."),type:'".$row['Type']."',name:'".$row['nom'] ." ".$row['prenom']."',adresse:'".$row['adresse']."',idClient:".$row['IdClient'].",lat:".$row['latitude'].",lng:".$row['longitude'].",activite:'".$row['DsgActivite']."',icon:'".$row['icone']."'},";
	...
	if( $stmt2 === false ) {
			$errors = sqlsrv_errors();
			echo "Error : ".$errors[0]['message'] . " <br/> ";
			return;
	}
...
<div Style="display: flex;">
		...
		<div>			
			<select multiple="multiple" id="classe" name="classe[]" Class="Select claqu'invitésse" style="width:280px">
...
$(document).ready(function() {
		//$.validator.messages.required = '';
		 var map;   var markerInClick;  
		 if(!$.cookie("client"))
			{
				var clt={};
				$.cookie("client",JSON.stringify(clt));
			}
  		$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire.php?map');
				$('#boxClient').dialog({
					autoOpen		:	false,
					width			:	800,
					height			:	400,
...
function getInventaire(){
	url="inventaire.php?affArti&VideFam";
	...
	var form="";
	    ...
	    $(form).validate({
                                   ...
                                   rules: {
                                                ...
                                                'Qte': "required"										//	'Colisage': "required"
                                           }  
