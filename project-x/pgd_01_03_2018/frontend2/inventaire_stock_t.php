...
//parcourir($_COOKIE['lignesFam']);return;
foreach($_COOKIE['lignesFam'] as $v){//famille
...
<ul class="bxslider" style="margin:0;padding:0;">
							...
							//	parcourir($_COOKIE['lignesFam']);return;
							$key ="";
				if( (isset($_COOKIE['lignesFam'])) && (count($_COOKIE['lignesFam'])!=0))  {//famille
				//echo $_SESSION['IdFamille'];return;
				
					$i=1;
					foreach($_COOKIE['lignesFam'] as $v){// marque
							if(is_array($v)){ 
...
if (isset($_GET['affMarque'])){
	//	parcourir($_COOKIE['lignesCat']);return;
	?>
	<DIV style="    align-items:center;" class="headVente">
	<div style="float:left; width:80%">
		<a href="index.php">	
		<img src="../images/home.png" height="64" width="64" style="float:left;" /> </a>
			<div style="padding-top:12px" 	>
					&nbsp; >&nbsp; 
				<span style="" class="TitleHead" onclick="rechercherMarque()"> <?php  echo $trad['label']['Inventaire'];?></span> &nbsp; 
		</div>
	</div>
...
//unset($_COOKIE['lignesFam']);

if(!isset($_COOKIE['lignesFam'])){
...
parcourir($_COOKIE['lignesFam']);
?>	

		<?php 
unset($_COOKIE['lignesCat']);
	//parcourir($_COOKIE['lignesCat']);return;
?>	
...
$(document).ready(function () {
		var url='inventaire_stock_t.php?ConsultCmd';	
		$('#ListInv').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
	var i = 0;
...
<ul class="bxslider" style="margin:0;padding:0;">
							<?php 
							$key ="";
				if( (isset($_COOKIE['lignesFam'])) && (count($_COOKIE['lignesFam'])!=0))  {
					?>
					<?php	
					$i=1;
				///parcourir($_COOKIE['lignesFam']);return;
						foreach($_COOKIE['lignesFam'] as $t){	//	parcourir marque
							if(is_array($t)){	
...
<ul class="bxslider" style="margin:0;padding:0;">
				...
				if( (isset($_COOKIE['lignesFam'])) && (count($_COOKIE['lignesFam'])!=0))  {
					...
					<?php	
					$i=1;
						foreach($_COOKIE['lignesFam'] as $v){	//// parcourir marque				
...
<div id="resReg"></div>
	...
	<script language="javascript" type="text/javascript">	
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'inventaire_stock_t.php?TerminerCmd',
								method			:	'post'
							});
		patienter('resReg');
		}
...
/*if($_POST['TypeReg']=="Cheque") { $Bq=$_POST['Bq'] ;$numCheque=$_POST['Num'];}
	...
	if( (isset($_COOKIE['lignesCat'])) && (count($_COOKIE['lignesCat'])!=0))  {
...
//----------------------Add Detail facture --------------------------//
  foreach($_COOKIE['lignesCat'] as $ligne=>$contenu){
		$nbrRef+=1;		  
		$UniteVente="";
...
if (isset($_GET['VerifStock'])){
	...
	$QteDispo=tofloat($QteEntree)-tofloat($QteSortie);
			...
			//	echo ("----QteCmd ".$QteCmd."--------- QteEntree ".$QteEntree."--------- QteSortie ".$QteSortie."--------- QteDispo ".tofloat($QteDispo));					
					if((tofloat($QteCmd)) > $QteDispo) echo "0";
					else echo "1";
	   exit;				
}
...
if(isset($_COOKIE['lignesCat']) && count($_COOKIE['lignesCat']) != 0){		
	...
	<DIV class="ListeCmd">
	<div class="enteteL" >
		<div  class="divColisageL" style="width:120px" ><?php echo $trad['label']['Ref'];?>  </div>
  	...
  	<div style="height:350px;overflow:scroll;" ><!--height:585px;-->
			...
			$k=0;$Total=0;
			//parcourir($_COOKIE['lignesCat']);return;
			foreach($_COOKIE['lignesCat'] as $ligne=> $row){
				$k++;
			...
			$Total=0;
				?>	
				<div class="divColisageL" style="width:120px"><?php  echo $row['Reference'];?>  </div> 
				<div class=" <?php echo $c;?>" onclick="getArticle('<?php  echo $row['IdArticle'];?>','List')">
						...
						<div class="divQteL"  style="width:120px" >  <span class="nbr"><?php 
					echo $row['Qte']." ".$trad['label']['Boxs'];
					echo $row['QtePcs']." ".$trad['label']['NbrPiece'];?> </span></div>
...
<div class="divLeft" style="width:100%">
		...
		<div class="divRight" style="width:100%">
			<input type="button" value="<?php echo $trad['button']['enregistrer'];?>" class="btnCmd" onclick="ConsultCmd()">
		</div>
		</div>
	</form>
<?php
	...
	if( (isset($_COOKIE['lignesCat'])) && (count($_COOKIE['lignesCat'])!=0))  {
		...
		$Total=0;
			  foreach($_COOKIE['lignesCat'] as $ligne=>$contenu){
				...
				$Total+=$contenu["Qte"]*$contenu["Colisage"]*$contenu["PV"];
					...
					{	
								...
								$ligneArray["QtePcs"]=$_POST["QtePcs"];
								...
								$_COOKIE['lignesCat'][$IndexLigne]= $ligneArray;
			  ...
			  if($t!=0){
			  			...
			  			$IndexLigne=count($_COOKIE['lignesCat']);							
										...
										<form id="formAddArt" method="post" name="formAddArt"> 
											<TABLE  dir="<?php echo $_SESSION["dir"];?>"  border="0" width="100%" class="table" cellspacing="2" cellpadding="7">
												...
												<?php if (isset($_GET['list'])){
													...
													$QtePcs=$_SESSION["lignesCat"][$key]["QtePcs"];
												...
												<tr><TD  style="display:none;"><?php  echo $trad['label']['PV'];?>:</td><td   style="display:none;" align="<?php echo $_SESSION["align"];?>">
												<span class="prix">
												...
												<tr><TD style="display:none;"><?php  echo $trad['label']['unite'];?>:</td>
												<td height="70" style="display:none;" align="<?php echo $_SESSION["align"];?>">
												...
												<tr><TD><?php  echo $trad['label']['Qte']." ".$trad['label']['Boxs'];?>:</td><td Valign="top" align="<?php echo $_SESSION["align"];?>"> 
												...
												<input type="button" class=" qtyplus"  style="padding-right:0" id="qtyplus" onclick="Plus('<?php echo $i;?>','Qte')"
												value="+" id="btnp">&nbsp;
												<input type="button" class=" qtyplus" style="padding-right:0" id="qtyminus" onclick="Moins('<?php echo $i;?>','Qte')" value="-"></td></tr>
												
												<tr><TD><?php  echo $trad['label']['Qte']." ".$trad['label']['NbrPiece'];?>:</td><td Valign="top" align="<?php echo $_SESSION["align"];?>"> 
												<input type="text" value="<?php echo $QtePcs;?>" name="QtePcs" onkeypress="return isEntier(event) " 
												class="Qte ConvertDecimal nbr" 
												Style="text-align:<?php echo $_SESSION["align"];?>"
												size="4" id="QtePcs<?php echo $i;?>">
												<input type="button" class=" qtyplus"  style="padding-right:0" id="qtyplus" onclick="Plus('<?php echo $i;?>','QtePcs')"
												value="+" id="btnp">&nbsp;
												<input type="button" class=" qtyplus" style="padding-right:0" id="qtyminus" onclick="Moins('<?php echo $i;?>','QtePcs')" value="-"></td></tr>
												
												<TR>	
												<td   style="display:none;"Valign="middle" colspan="2" height="70">
...
function ConsultCmd(){
	 var Verif="";
	$.get("inventaire_stock_t.php?VerifSession", function(response) {
      Verif = response;
...
function getArticle(idArticle,list){
	if (list === undefined || list === null) {
		var url='inventaire_stock_t.php?getArticle&&idArticle='+idArticle;	
		$('#boxArticle').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');
	}
	else{
		//pour afficher liste
		var url='inventaire_stock_t.php?getArticle&&list&&idArticle='+idArticle;	
		$('#boxArticle').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');
	}
}
...
//$("#txtTotal").val("<?php echo number_format("0.00", 2, '.', ' '); ?>");
	//$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire_stock_t.php?affGamme&&VideFam');
	$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire_stock_t.php?affMarque&&VideFam');
		//$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire_stock_t.php?TypeVente');
...
function AfficheGamme1(CurrentSlide){
					...
					var url='inventaire_stock_t.php?affGamme'	}
					else {
							var url='inventaire_stock_t.php?affGamme&CurrentSlide='+CurrentSlide;	
					}			
...
function AfficheGamme1(id,CurrentSlide){
					...
					if (CurrentSlide === undefined || CurrentSlide === null) {
					var url='inventaire_stock_t.php?affGamme&&Id='+id;	}
					else {
							var url='inventaire_stock_t.php?affGamme&Id='+id+'&CurrentSlide='+CurrentSlide;	
					}
...
function AfficheDetailGamme(id){
	//alert('lll');
	/*var url='inventaire_stock_t.php?aff&&Id='+id;	
	$('#box').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');*/
	var current = slider.getCurrentSlide();
	var url='inventaire_stock_t.php?affArti&Id='+id+'&CurrentSlide='+current;	
	$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url);

}
function rechercherFam(id){
	
		var url='inventaire_stock_t.php?affFam&&Id='+id	;
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
}
function AfficheSousFam(id){
		//alert(id);
		var url='inventaire_stock_t.php?affSousFam&&Id='+id;	
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);

	}
	function AfficheMarque(){
		var url='inventaire_stock_t.php?affMarque';	
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);

	}
	/*
function AfficheMarque(id){
		var url='inventaire_stock_t.php?affMarque&&Id='+id;	
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
...
function FermerFenetre(){
	alert("fermer");
	var url='inventaire_stock_t.php?FermerFenetre';	
	//$("#box").load(url);
}
function ValideCmdEtape1(){
	/*$('#formCmd').ajaxSubmit({
		target:'#res',
		url:'inventaire_stock_t.php?ValideCmd',
			method			:	'post'
		});
		patienter('res');*/
...
$('#formRes').load('paiement.php?ChoixTypeReg');*/
		...
		//$('#box').dialog('close');
		$('#res').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire_stock_t.php?TerminerCmd');
}
					...
					if(r)	{
							var Verif="";							 
							 // verification du stock vendeur pour l'article selectionné
							 	$(form).ajaxSubmit({
									   url : 'inventaire_stock_t.php?VerifStock',
									   type : 'POST',
									...
									//	alert(Verif);
											...
											$(form).ajaxSubmit({
																target			:	'#resAdd',
																url				:	'inventaire_stock_t.php?goAddArti',
																method			:	'post'
															});
															return false;
											}else 
...
function GetCatalogue(TypeVente){			 
	var url='inventaire_stock_t.php?affFam&&VideFam&&IdTypeVente='+TypeVente;
	$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
}
...
function RechArticle (){
		var Ref=$("#Ref").val();
		var url='inventaire_stock_t.php?getArticle&&Rech&&Ref='+Ref;	
		$('#boxArticle').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');
		
		}
function  AffFormRech(){
	$('#boxRech').load("inventaire_stock_t.php?FormRech").dialog('open');
}	
</script>
