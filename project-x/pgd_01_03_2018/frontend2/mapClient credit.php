...
/*$sql = "SELECT  dbo.Distance(Latitude, Longitude, ".$_GET['lat'].", ".$_GET['lng'].") Distance, 
...
INNER JOIN  activites a ON c.IdActivite=a.IdActivite
 ...
 and  dbo.Distance(Latitude, Longitude, ".$_GET['lat'].", ".$_GET['lng']." ) <=60 and
 c.departement in
...
ORDER BY dbo.Distance(Latitude, Longitude,".$_GET['lat'].", ".$_GET['lng'].") ASC
";*/
//unset($_SESSION['tabClt']);
$tabClt=array();
...
//parcourir($tabRech);
 ...
 <form id="formRechF" method="post" name="formRechF" > 
  ...
  <input type="hidden" value="<?php echo $_GET['lng'] ;?>" name="lng" />
			...
			onclick="rechercher()" style="border:none;padding:0 30px 5px 30px;" />	
		
	</form>
</div>	
	 <div id="listClt"  style="background:#fff;" >	  
	 </div>
...
$(document).ready(function() {	
	rechercher();
//$('#listClt').load('mapClient.php?ChargerClt');
});
function rechercher(){
		$('#formRechF').ajaxSubmit({
		target:'#listClt',
		url:'mapClient.php?ChargerClt',
		method:	'post'
	})
}
...
factures.idClient=".$_GET['idClient'].") AS ca ,(SELECT count(*) 
...
<table  width="87%" cellspacing="10" border="0" >
	...
	<tr>
		...
		$params3= array(				
			...
			$CreditClt=creditClient($params3,$conn);
			ECHO $CreditClt;
		if(($CreditClt)!=1) {?>
		<td align="<?php echo $_SESSION['align']; ?>">
			...
			<?php echo number_format($CreditClt, 2, '.', ' ');?><?php echo $trad['label']['riyal']; ?>
			 </div>
			 </td>
		<?php } ?>
		</td>
...
var Center = new google.maps.LatLng(33.577930, -7.641432 );  
 ...
 zoom: 17,		  
center: Center, //cette ligne été en commentaire         
 mapTypeId: 'roadmap'        }); 
 var currentPosition="";
 //Ce bloc été décommenter
...
var marker3 = new google.maps.Marker({   
		...
		map: map  ,
        position:Center//j'ai ajouté cette ligne pour fixé la position du markeur de la position acctuel
    });    
