...
$('#boxClient').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load("mapClient2.php?infoClient&idClient="+feature.idClient+"&from="+json1+"&to="+json2+"&distance="+distance).dialog('open');	
...
$(document).ready(function() {
		//$.validator.messages.required = '';
  		//$('#formRes').load('mapClient2.php?search');
		 var map; 
		...
		/*	   jQuery.each(arrDetailAdr, function(i, val) {
		    ...
		    for (ac = 0; ac < arrDetailAdr.length; ac++) {
					...
					if (arrDetailAdr[ac].types[0] == "locality"){ // city
								    ...
								    //alert(ville);
									$('#formRes').load('mapClient2.php?map&Ville='+ville);
...
function rechercher(){
		$('#formAdd').ajaxSubmit({target:'#formRes',url:'mapClient2.php?aff'})
