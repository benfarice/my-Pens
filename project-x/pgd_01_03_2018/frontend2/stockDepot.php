			...
			if(test==true) {		
			 jConfirm('<?php echo $trad['msg']['terminerOperation'];?>', "<?php echo $trad['titre']['Alert'];?>", function(r) {
					if(r)	{
							$('#formAdd').ajaxSubmit({
									target			:	'#result',
									url				:	'stockDepot.php?goupdate&idVnd='+IdVnd+'&idCmd='+IdCmd,
									method			:	'post'
							}); 
							return false;
			           }
...
function getarticle()
{
	var url='stockDepot.php?getCommande&getArticle';	
...
$sqlAr = "
					...
					$i=0;
													...
													if (!isset($gamme[$key])) {
														...
														$i=0;
															...
															if($gamme[$key]!=""){
																	...
																	$gamme[$key][$i]['qteStock']= $qteDispo % $row['colisagee'];
																	...
																	$qteDispoEnPcsRe=$qteDispoAvecReserve% $row['colisagee'];
																	$gamme[$key][$i]['qteDispoEnPcsRe']= $qteDispoEnPcsRe;
						...
						$_SESSION['Stock']=$gamme;
//}// fin if seesion

//parcourir($_SESSION['Stock']);return;
?>
			<!--div class="filtre">
...
<DIV class="ListeCmd">
	<div class="enteteL" >
			...
			<div style="width:140px;" class="divArticleL"  ><?php echo $trad['label']['Gamme'];?> </div>
			...
			<div style="width:200px;" class="divArticleL" ><?php echo $trad['label']['StockPrevente'];?>  </div>	
			<div style="width:200px;" class="divArticleL" ><?php echo $trad['label']['QteStock'];?>  </div>	
	</div>
	<div style="height:375px;overflow:scroll;" ><!--height:585px;-->
<?php 
$k=0;
//parcourir($_SESSION['Stock']);
foreach($_SESSION['Stock'] as $u=>$g){
$k++;
if($k%2 == 0) $c = "pair";
else $c="impair";
	if(is_array($g)){
...
<?php
 foreach($g as $article){	
		...
		?>
								...
								<div  class="divPv" style="width:200px;" align="left" > <?php  echo $article['RefArticle'];?>					
									</div>
									<div style="padding:10px;width:520px;" class="divPv" align="left" > 
										 <?php 
										 echo ucfirst(stripslashes(	wordwrap($article['DsgArticle'], 30, "\n")));
										  ?>					
									</div>
									<div class="divPv" style="padding:10px;width:100px;direction:ltr;text-align:right" > 									
									<?php  
								...
								//	echo number_format($valeur,2);
									...
									<?php if (($article['qteDispoEnBoiteRe']==0)&&($article['qteDispoEnPcsRe']==0)) echo "0"; else {
											if ($article['qteDispoEnBoiteRe']!=0)  echo floor($article['qteDispoEnBoiteRe'])." ".$trad['label']['Boxs'] ;?>  
										<?php if ($article['qteDispoEnPcsRe']!=0) echo "   ".number_format($article['qteDispoEnPcsRe'], 0, '.', ' ')." ".$trad['label']['NbrPiece'] ;
										}												
									?>									</div> 
									
									<div class="divPv" style="padding:10px;width:250px;direction:ltr;text-align:right" >
								  
									<?php if (($article['qteStock']==0)&&($article['qteDispoEnBoite']==0)) echo "0"; else {
											if ($article['qteDispoEnBoite']!=0)  echo floor($article['qteDispoEnBoite'])." ".$trad['label']['Boxs'] ;?>  
										<?php if ($article['qteStock']!=0) echo "   ".number_format($article['qteStock'], 0, '.', ' ')." ".$trad['label']['NbrPiece'] ;
										}							
...
$(document).ready(function() {
		//$.validator.messages.required = '';
  		$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('stockDepot.php?aff');
...
function rechercher(){
		$('#formAdd').ajaxSubmit({target:'#formRes',url:'stockDepot.php?aff'})
