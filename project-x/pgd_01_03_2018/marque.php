...
if(isset($_GET['goAdd'])){
	$error="";
			...
			/**********************Controle doublon**********code Famille************************/
$sql = "SELECT * FROM ".$tableInser." WHERE codeMarque=? ";
$param = array($_POST['codeMarque']);
...
$IdAffectation = "" ;
	...
	?>
				<script type="text/javascript"> 
					alert('Code Marque déja existant.');
				</script>
	<?php
	return;
	}
	...
	$reqInser = "INSERT INTO ".$tableInser." ( codeMarque,Designation,chemin ) values (?,?,?) ";
	$params1= array($_POST['codeMarque'],$_POST['DsgMarque'],$target_path) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	<td align="right"><div class="etiqForm" id="" > <strong>Code Marque</strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="codeMarque"  id="codeMarque" size="30" tabindex="1"  />
            </td>
         
                <td><div class="etiqForm" id="" > <strong>Désignation Marque</strong> : </div>
            </td>
            <td>
          <!--  <input class="FormAdd1" type="text" name="DsgTransport" id="DsgTransport" size="25"  control="1" tabindex="6"  />-->
           <textarea  rows="1" cols="30"  tabindex="2"   class="FormAdd1" name="DsgMarque" style="text-align:left;resize:None;" >
</textarea>
            </td>
             </tr>
			<tr>
			...
			<td height="20" > 
			<input class="file" type="file" name="file" control="1" id="file" size="25" tabindex="1" onchange="readURL(this);"  value="" /></td>
			<td colspan="2" > 
				<div class="DivFile" style="border:none;"  >
				<img  class=" tof" height="113" width="108" style="display:inline-block;" />
			</div>
			</td>			
			</tr>			 
			<tr><td colspan="4"  height="20" > <div class="msgErreur">&nbsp;</div></td></tr>	  
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT idMarque,codeMarque,Designation FROM marques m";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['codeMarque']) && ($_POST['codeMarque']!='') )
	{  
	   $sqlA .=" where codeMarque like ? " ;
	   $params = array("%".$_POST['codeMarque']."%");
	}
	//ECHO $sqlA."<br>";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%">Code Marque </td>
        <td width="50%"> Désignation Marque </td>
...
<!--<div id="cList">-->
	<?php
		$i=0;
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="left"  > <?php echo $row['codeMarque']; ?> </td>
				<td align="left" > <?php 	echo $row['Designation'];?> </td>
				<!--td align="center">
					<span class="boutons"> 
					<input type="button" title="Modifier" action="mod" class="b" onClick="modifier('<?php echo $row['codeMarque']; ?>');" />  
					</span>
			  </td>			
			  <td align="left">
				<input type="checkbox" class="checkLigne" name="<?php	echo $row['codeMarque']; ?>" value="<?php	echo $row['codeMarque']; ?>" />
			  </td-->
			  </tr>
			 <?php
			$i++;
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'marque.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" > Code Marque : </div>				</td>
					<td width="30%">
					<!---<input class="formTop" name="COLBQ" type="hidden" size="30" value="4"/>-->
					<div align="left">
				<input class="formTop"  name="codeMarque" type="text" size="30" />						</div>
										</td>
			...
			<tr height="20">
			  ...
			  <td width="12%">Pages : <span id="cont_pages">
				...
				<td width="30%">Crit&egrave;re de tri : 
				  ...
				  <option value="codeMarque">  </option>
				<option value="DsgMarque">Désignation Marque </option>				
				</select>
		  	  </td>
...
$(document).ready(function(){	
  			$('#formRes').load('marque.php?aff');
				$('#boxFamille').dialog({
					...
					closeOnEscape	:	true,
					draggable		:	true,
					title			:	'Ajout / Modification de famille',
					buttons			:	{
						"Annuler"		: function(){
							$(this).dialog('close');
						},
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'marque.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'marque.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='marque.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='marque.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
