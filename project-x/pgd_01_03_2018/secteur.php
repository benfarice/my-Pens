...
$IdAffectation = "" ;
	...
	$params1= array(addslashes(mb_strtolower(securite_bdd($_POST['CodeSecteur']), 'UTF-8')),addslashes(mb_strtolower(securite_bdd($_POST['DsgSecteur']), 'UTF-8')),$_POST['Ville'],1) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	<td><div class="etiqForm" id="" > <strong><?php echo $trad['label']['codeSecteur']; ?></strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="CodeSecteur"  id="CodeSecteur" tabindex="1"  style="width:220px"/>
            </td>
          </tr>
			 <tr>
          ...
          <!--  <input class="FormAdd1" type="text" name="DsgTransport" id="DsgTransport" size="25"  control="1" tabindex="6"  />-->
           <textarea  rows="1" cols="42"  tabindex="2"   class="FormAdd1" name="DsgSecteur" style="text-align:<?php echo $_SESSION['align'] ; ?>;resize:None;width:220px" >
</textarea>
            </td>
             </tr>
			 <tr>
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT d.iddepartment,d.codeDepartement,d.Designation AS secteur , v.Designation ville FROM departements d INNER JOIN villes v ON v.idville = d.idVille";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['CodeSecteur']) && ($_POST['CodeSecteur']!='') )
	{  
	   $sqlA .=" where codeDepartement like ? " ;
	   $params = array("%".$_POST['CodeSecteur']."%");
	}
	//ECHO $sqlA."<br>";
	...
	$sqlC = " ORDER BY iddepartment desc";//$cTri $oTri ";//LIMIT $min,$max ";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%"> <?php echo $trad['label']['codeSecteur']; ?> </td>
        <td width="50%"> <?php echo $trad['label']['Dsg']; ?> </td>
	...
	<?php
		$i=0;
			...
			<tr  class="<?php echo $c; ?>">
				...
				<td align="<?php echo $_SESSION['align'] ; ?>" > <?php 	echo $row['secteur'];?> </td>
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'secteur.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['parametrage']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;<?php echo $trad['label']['secteur']; ?>&nbsp;</div>
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" ><?php echo $trad['label']['codeSecteur']; ?> : </div>				</td>
					<td width="30%">
					<!---<input class="formTop" name="COLBQ" type="hidden" size="30" value="4"/>-->
					<div align="left">
				<input class="formTop"  name="CodeSecteur" type="text" size="30" />						</div>
										</td>
...
$(document).ready(function(){	
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'secteur.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'secteur.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='secteur.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='secteur.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
