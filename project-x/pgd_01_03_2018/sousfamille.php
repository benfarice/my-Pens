...
$sql = "SELECT * FROM ".$tableInser." WHERE codeSousFamille=? ";
$param = array($_POST['CodeFamille']);
...
$IdAffectation = "" ;
	...
	?>
				<script type="text/javascript"> 
					alert('<?php echo $trad['msg']['codeSsFamilleexist']; ?>');//alert('Code Sous Famille déja existant.');
				</script>
	<?php
	return;
	}
	...
	$reqInser = "INSERT INTO ".$tableInser." ( codeSousFamille,Designation,idFamille,idDepot ) values (?,?,?,?) ";
	
	$params1= array(addslashes(mb_strtolower(securite_bdd($_POST['CodeFamille']), 'UTF-8')),addslashes(mb_strtolower(securite_bdd($_POST['DsgFamille']), 'UTF-8')),$_POST['Famille'],1) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	<td><div class="etiqForm" id="" > <strong><?php echo $trad['label']['codeSsFamille']; ?></strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="CodeFamille"  id="CodeFamille" tabindex="1"  style="width:220px"/>
            </td>
          </tr>
			 <tr>
          ...
          <!--  <input class="FormAdd1" type="text" name="DsgTransport" id="DsgTransport" size="25"  control="1" tabindex="6"  />-->
           <textarea  rows="1" cols="42"  tabindex="2"   class="FormAdd1" name="DsgFamille" style="text-align:
<?php $_SESSION['align'] ; ?>;resize:None;width:220px" >
</textarea>
            </td>
             </tr>
			 <tr>
			  <td><div class="etiqForm" id="" > <strong><?php echo $trad['label']['Famille']; ?> </strong> : </div>
            </td>
            <td>
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT sf.codeSousFamille,sf.Designation ssfamille,f.Designation AS famille  FROM sousfamilles sf INNER JOIN familles f ON f.idFamille = sf.idFamille";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['CodeFamille']) && ($_POST['CodeFamille']!='') )
	{  
	   $sqlA .=" where codeSousFamille like ? " ;
	   $params = array("%".$_POST['CodeFamille']."%");
	}
	//ECHO $sqlA."<br>";
	...
	$sqlC = " ORDER BY idSousFamille desc";//$cTri $oTri ";//LIMIT $min,$max ";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%"><?php echo $trad['label']['codeSsFamille'];?></td>
        <td width="50%"> <?php echo $trad['label']['SousFamille'];?> </td>
        <td width="50%"> <?php echo $trad['label']['Famille'];?> </td>		
...
<!--<div id="cList">-->
	<?php
		$i=0;
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="<?php $_SESSION['align'] ; ?>"  > <?php echo $row['codeSousFamille']; ?> </td>
				<td align="<?php $_SESSION['align'] ; ?>" > <?php 	echo $row['ssfamille'];?> </td>
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'sousfamille.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['parametrage']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;<?php echo $trad['titre']['SousFamille']; ?> &nbsp;</div>
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" ><?php echo $trad['label']['codeSsFamille']; ?> : </div>				</td>
					<td width="30%">
					<!---<input class="formTop" name="COLBQ" type="hidden" size="30" value="4"/>-->
					<div align="left">
				<input class="formTop"  name="CodeFamille" type="text" size="30" />						</div>
										</td>
...
<script language="javascript" type="text/javascript">
$(document).ready(function(){	
  			$('#formRes').load('sousfamille.php?aff');
				$('#boxFamille').dialog({
					autoOpen		:	false,
					width			:	600,
					height			:	400,
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'sousfamille.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'sousfamille.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='sousfamille.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='sousfamille.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
