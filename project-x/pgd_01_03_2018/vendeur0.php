...
if(isset($_GET['goAdd'])){
$error="";
//print_r($_POST);print_r($_FILES);echo ($_FILES['file']['name']);return;
...
filter: true,placeholder:'S&eacute;lectionnez la catégorie du permis',single:true,maxHeight: 100,width:220
});
...
<form id="formAdd" action="NULL" method="post"  name="formAdd1"  enctype="multipart/form-data" > 	
	<table width="100%" border="0" align="center" cellpadding="5" cellspacing="10">
		...
		<tr>
        	<td>
			 <div class="etiqForm" id="" ><strong>Date Livraison Permis </strong> : </div>
            </td>
            <td>
			 <input class="FormAdd1" type="text" name="Dpermis" maxlength="10" onChange="verifier_date(this);"  id="Dpermis" size="30" tabindex="1"  />
            </td>
			<td>
			 <div class="etiqForm" id="" > <strong>Date d'embauche</strong> : </div>
            </td>
            <td>
		...
		<tr>
        	<td>
			 <div class="etiqForm" id="" >N°.CNSS <strong></strong> : </div>
            </td>
            <td>
        ...
        <tr>		
			<td valign="top">
			 <div class="etiqForm" id="" > Photo<strong></strong> : </div>
            </td>
            <td valign="top">
               <input class="file" type="file" name="file" control="1" id="file" size="25" tabindex="1" onchange="readURL(this);"  value="" />
			 </td>
			<td colspan="2">		
...
$sqlA = "SELECT * FROM vendeurs";
	...
	{	$sqlA .=" where codeVendeur like ? " ;
	   $params = array("%".$_POST['codeV']."%");
	}
	//ECHO $sqlA."<br>";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="100%" border="0">
      <tr class="entete">
		<td width="10%">Code Vendeur</td>	  
        <td width="15%">Nom</td>
        <td width="15%">Prénom</td>
		<td width="20%">adresse</td>
        <td width="10%">Télèphone</td>  
...
<!--<div id="cList">-->
	<?php
		...
		while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){		
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="left"  > <?php echo $row['codeVendeur']; ?> </td>			
				<td align="left"  > <?php echo $row['nom']; ?> </td>
				<td align="left" > <?php 	echo $row['prenom'];?> </td>
		...
		<div id="formRech" style="">	
			<table width="100%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" >Code Vendeur: </div>				</td>
					<td width="30%">
...
$(document).ready(function(){	
  		$('#formRes').load('vendeur.php?aff');
				$('#boxClient').dialog({
					...
					closeOnEscape	:	true,
					draggable		:	true,
					title			:	'Ajout / Modification des vendeurs',
					buttons			:	{
						"Annuler"		: function(){
							$(this).dialog('close');
						},
