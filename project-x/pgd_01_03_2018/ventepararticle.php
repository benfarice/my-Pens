...
$sqlA = " SELECT fa.idFamille,fa.codeFamille,fa.Designation AS famille,a.CB,a.Reference, a.Designation  AS article,df.UniteVente,
 sum(df.qte) AS qte, sum(df.ttc) AS ttc 
FROM factures f 
...
INNER JOIN familles fa ON fa.idFamille=s.idFamille".$where;

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	if(isset($_POST['Famille']) && ($_POST['Famille']!='') )
	{	$sqlA .=" AND fa.idFamille like ? " ;
	   $params = array("%".$_POST['Famille']."%");
	}
	$sqlA .=" GROUP BY fa.idFamille,fa.codeFamille,fa.Designation,a.CB,a.Reference, a.Designation ,df.UniteVente";
//echo 	$sqlA;
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="100%" border="0" cellspacing="2">
       <tr class="entete">
	    <td width="10%"><?php echo $trad['label']['codeFamille']; ?></td>  
        <td width="10%"><?php echo $trad['label']['Famille']; ?></td>
        <td width="10%"><?php echo $trad['label']['reference']; ?></td>		
		<td width="10%"><?php echo $trad['label']['codeBarre']; ?></td>
        <td width="15%"><?php echo $trad['label']['Article']; ?></td>
		<td width="10%"><?php echo $trad['label']['unite']; ?></td>		
		<td width="20%"><?php echo $trad['label']['qteVendu']; ?></td>
        <td width="20%"><?php echo $trad['label']['ValTTC']; ?></td>
	...
	//v.cin ,v.nom + ' ' + v.prenom AS 'Nom' ,fa.idFamille,fa.codeFamille,fa.Designation AS famille, a.CB , a.Designation  AS article,sum(df.qte) AS qte, sum(df.ttc) AS ttc ,
		...
		while($row = sqlsrv_fetch_array($resAff, SQLSRV_FETCH_ASSOC)){		
								...
								if (!isset($famille[$key])) {
									$famille[$key] = array();
									$famille[$key]['idFamille']=$row['idFamille'];
									$famille[$key]['codeFamille']=$row['codeFamille'];
									$famille[$key]['famille']=$row['famille'];									
									
								} 
								$keyArticle= $row['Reference'];
								if (!isset($famille[$key][$keyArticle])) {
									$famille[$key][$keyArticle] = array();
									$famille[$key][$keyArticle] ['CB']=$row['CB'];	
...
//	echo "<pre>";	print_r($famille);echo "</pre>";return;
	$sum_famille_qte=0;$sum_famille_ttc=0;$sum_vnd_qte=0;$sum_vnd_ttc=0;
		foreach($famille as $u=>$fam){
			...
			<tr class="pair" style="height:50px;font-size:14px;">
				<td align="<?php $_SESSION['align'] ; ?>" > <strong><?php echo $fam['codeFamille']; ?> </td>
				<td align="<?php $_SESSION['align'] ; ?>" colspan="7" > <strong> <?php echo $fam['famille'];?> </strong></td>
			</tr>
						...
						<td colspan="4"></td>
						<td align="right" colspan="2"><strong><?php echo $trad['label']['total']; ?>   <?php echo $fam['famille']; ?> : </strong></td>
						<td  style="direction: ltr;" align="<?php echo ($_SESSION['lang'] == 'ar' ) ? 'left' : 'right'; ?>">
						<?php echo number_format($sum_famille_qte,0," "," "); ?>
						</td>	
						<td align="<?php echo ($_SESSION['lang'] == 'ar' ) ? 'left' : 'right'; ?>"  style="direction:ltr;">
						<?php echo  DHS($sum_famille_ttc); ?></td>						
						</tr>
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['statistic']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;<?php echo $trad['Menu']['venteFamille']; ?>&nbsp;</div>
...
<input name="DATED" type="hidden" value=""/>	
			  ...
			  <input name="button" type="button"  onClick="rechercher();" value="<?php echo $trad['button']['rechercher']; ?>" class="bouton32" action="rech" title="<?php echo $trad['button']['rechercher']; ?> " />
				...
				<tr>
					...
					<div class="etiqForm" id="SYMBT" ><strong> <?php echo $trad['label']['Famille'] ; ?>:</strong> </div>				</td>
					<td width="30%">
...
$('#Famille').multipleSelect({
	   filter: true,placeholder:'<?php echo $trad['label']['selectFamille']; ?>',single:true,maxHeight: 100
  		...
  		$('#formRes').load('ventepararticle.php?aff');
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'ventepararticle.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'ventepararticle.php?rech'})
		clearForm('formRechF',0);
	}
function effacer()
	{
	    $("select#Famille").multipleSelect("uncheckAll");
		
	
	}
function ajouter(){
		$('#act').attr('value','add');
		var url='ventepararticle.php?add';	
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='ventepararticle.php?mod&ID='+id;
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
