...
if(isset($_GET['goAdd'])){
	$error="";
			...
			/**********************Controle doublon**********code Ville************************/
$sql = "SELECT * FROM ".$tableInser." WHERE codeVille=? ";
$param = array($_POST['CodeVille']);
...
$IdAffectation = "" ;
	...
	?>
				<script type="text/javascript"> 
					alert('<?php echo $trad['msg']['codeVilleexist']; ?>');
				</script>
	<?php
	return;
	}

	$reqInser = "INSERT INTO ".$tableInser." ( codeVille,Designation,idRegion,iddepot ) values (?,?,?,?) ";
	
	$params1= array(addslashes(mb_strtolower(securite_bdd($_POST['CodeVille']), 'UTF-8')),addslashes(mb_strtolower(securite_bdd($_POST['DsgVille']), 'UTF-8')),$_POST['Region'],1) ;
...
$('body').on('change', '#Region', function() {
	 	var Region =$('#Region').val(); <?php //echo $row["IdVille"];?>
		if(Region!="") {
					$('div.Region').removeClass('erroer');
					$('div.Region button').css("border","1px solid #ccc").css("background","#fff");
		}
	 });
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	<td><div class="etiqForm" id="" > <strong> <?php echo $trad['label']['codeVille'];?></strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="CodeVille"  id="CodeVille" tabindex="1"  style="width:220px"/>
            </td>
          </tr>
			 <tr>
          ...
          <!--  <input class="FormAdd1" type="text" name="DsgTransport" id="DsgTransport" size="25"  control="1" tabindex="6"  />-->
           <textarea rows="1" cols="42" tabindex="2" class="FormAdd1" name="DsgVille" style="text-align:<?php echo $_SESSION['align'] ; ?>;resize:None;width:220px" ></textarea>
            </td>
             </tr>
			 <tr>
            ...
            <!--select  name="Famille" id="Famille"  multiple="multiple" tabindex="3" style="width:220px;" class="Select Famille">
				...
				<option>Famille2</option>
			 	...
			 	<select name="Region" id="Region"  multiple="multiple" tabindex="3" class="Select Region" style="display:visible;width:220px;">
		
                         <?php $sql = "select idRegion, Designation from regions ";
                       $reponse=sqlsrv_query( $conn, $sql, array(), array( "Scrollable" => 'static' ) );         
                               while ($donnees =  sqlsrv_fetch_array($reponse))
                               {
                               ?>
                           <option value="<?php echo $donnees['idRegion'] ?>"><?php echo $donnees['Designation']?></option>
                         <?php
                          }
                         ?>
				</select>
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT idville,v.codeVille,v.Designation AS ville, r.Designation AS region FROM villes v INNER JOIN regions r ON r.idRegion = v.idRegion";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['CodeVille']) && ($_POST['CodeVille']!='') )
	{  
	   $sqlA .=" where v.codeVille like ? " ;
	   $params = array("%".$_POST['CodeVille']."%");
	}
	//ECHO $sqlA."<br>";
	...
	$sqlC = " ORDER BY idville desc";//$cTri $oTri ";//LIMIT $min,$max ";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%"><?php echo $trad['label']['codeVille']; ?> </td>
        <td width="50%"><?php echo $trad['label']['ville']; ?></td>
...
<!--<div id="cList">-->
	<?php
		$i=0;
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="<?php echo $_SESSION['align'] ; ?>"  > <?php echo $row['codeVille']; ?> </td>
				<td align="<?php echo $_SESSION['align'] ; ?>" > <?php 	echo $row['ville'];?> </td>
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'ville.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" ><?php echo $trad['label']['codeVille']; ?> : </div>				</td>
					<td width="30%">
					<!---<input class="formTop" name="COLBQ" type="hidden" size="30" value="4"/>-->
					<div align="left">
				<input class="formTop"  name="CodeVille" type="text" size="30" />						</div>
										</td>
...
$(document).ready(function(){	
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'ville.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'ville.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='ville.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='ville.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
