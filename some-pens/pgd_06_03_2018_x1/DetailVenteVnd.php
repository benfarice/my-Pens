				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'DetailVenteVnd.php?delPlusieursArticle',clearForm:false});		
						}
					});
				}			
		}	
...
$(document).ready(function(){	
  		//$('#formRes').load('DetailVenteVnd.php?aff');
...
filter: true,placeholder:'S&eacute;lectionnez le vendeur',single:true,maxHeight: 100
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'DetailVenteVnd.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
...
function telecharger(){
		...
		var vendeur=$("select#Vendeur").multipleSelect("getSelects");	
		var mois=$("select#Mois").multipleSelect("getSelects");	//.multipleSelect("getSelects", "text");
		//window.open('DetailVenteVnd.php?goPdf&Vendeur='+vendeur+'&Mois='+mois, '_blank');	
		alert("here");
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'DetailVenteVnd.php?goPdf&Vendeur='+vendeur+'&Mois='+mois})
		clearForm('formRechF',0);
	}
function ajouter(){
		$('#act').attr('value','add');
		var url='DetailVenteVnd.php?add';	
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='DetailVenteVnd.php?mod&ID='+id;
		$('#boxArticle').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
