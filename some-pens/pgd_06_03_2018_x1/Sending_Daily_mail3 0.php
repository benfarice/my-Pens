...
$les_emails = array("oudani.ayoub@gmail.com","imzoughene@outlook.com","imzou01@gmail.com","benfarice@gmail.com","direction@electroprotect.ma","lahloureda69@gmail.com"
);
$les_pdfs =  array();
$message_div = "";
function sendMail_daily(array $les_emails,array $les_pdfs){
	...
	$mail->Host = 'tls://smtp.gmail.com:587';	//Sets the SMTP hosts of your Email hosting, this for Godaddy
...
cast(f.date as date) = '".date('m/d/Y')."' group by v.idVendeur";
$params0 = array();
...
on v.idville = dpt.IdVille where cast(fa.date as date) = '".date('m/d/Y')."'
...
where f.idVendeur = $row[id] and cast(f.date as date) = '".date('m/d/Y')."'";
//echo $sql_espece;
$params = array();
...
COUNT(a.IdArticle) as nbr_ref from detailFactures dtf  inner join
 ...
 cast(f.date as date)='".date('m/d/Y')."' group by m.Designation";
//echo $sql_marque;
$params_marq = array();
