			...
			//---------------------------select qteChargementGlobal--------------------------------//
				...
				if( ($error=="")) {
					...
					<table border="0" width="100%">
							...
							<td align="left" valign="top">Colisage:</td><td valign="top"><strong><?php 	echo  substr($Colisage,0,-1 ); ?></strong></td>
						</tr>
	...
	if(isset($_SESSION['lignes']) && count($_SESSION['lignes']) != 0){
        ...
        <td  colspan="2">
			...
			<form id="formLigne" action="" method="post" name="<?php echo $ligne; ?>" >
				...
				<td width="110"><?php echo $row['Unite']; ?><input type="hidden" value="<?php echo $row['IdArticle']; ?>" name="IdArticle[]"></td>
				<td width="110"  align="right">
				<?php 
				if((floatval($row['Qte'])*floatval($row['Colisage'])) < (floatval($row['QteDisp']) )) 
				...
				<td width="110"  align="right">
				<input type="text" name="QteDmdParPiece" STYLE="TEXT-align:right;background:none;border:0;"  size="7" Id="QteDmdParPiece<?php echo $k;?>" value="<?php echo floatval($row['QteDmdParPiece']); ?>" readonly>
				</td>
			
			
				<td width="110"  align="right"  class="positif"> 
					<input type="text" name="QteDisp" STYLE="TEXT-align:right;background:none;border:0;" size="7" Id="QteDisp<?php echo $k;?>" value="<?php echo floatval($row['QteDisp']); ?>" readonly>
			...
			//---------------------------select qteChargementGlobal--------------------------------//
				...
				while ($rc =  sqlsrv_fetch_array($stmt2))
				{
					...
					$qteChargementGlobal += floatval($rc["qte"]);
							...
							$ligneArray["QteDisp"]=floatval($qteEntreeGlobal)-$qteChargementGlobal;
	...
	if(isset($_POST['Colisage']) && $_POST['Colisage']!="" ) {
	   ...
	   if( (isset($_SESSION['lignes'])) && (count($_SESSION['lignes'])!=0))  {
		$t=0;	
			...
			$QteCharg=0;
			 ...
			 foreach($_SESSION['lignes'] as $ligne=>$contenu){
					...
					{					
						...
						$QteCharg+=floatval($contenu['QteDmdParPiece']);
						$t=$t+1;
			...
			$QteCharg=$QteCharg+ (floatval($_POST["Qte"])*$Col);
	  		//echo $QteDisp ;RETURN;	
			 ...
			 if (  $QteCharg > $QteDisp ) {
							...
							?>
						<script language="javascript" type="text/javascript">
							alert('Attention,la quantité chargée  doit être inférieur à la quantité du stock. '+<?php echo $QteDisp;?>);
 ...
 for( $i= 0 ; $i < count($_POST['QteCh']) ; $i++ )
{
	...
	$reqInser2 = "INSERT INTO  detailchargements([idArticle],[qte],[idColisage],IdChargement ) values (?,?,?,?)";
			$params2= array($_POST['IdArticle'][$i],$_POST['QteCh'][$i],$_POST['Colisage'][$i],$IdFiche) ;
