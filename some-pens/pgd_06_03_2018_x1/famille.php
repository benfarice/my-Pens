...
$sql = "SELECT * FROM ".$tableInser." WHERE codeFamille=? ";
...
$IdAffectation = "" ;
	...
	?>
				<script type="text/javascript"> 
					alert('<?php echo $trad['msg']['codeFamilleexist']; ?>');
				</script>
	<?php
	return;
	}
	...
	$reqInser = "INSERT INTO ".$tableInser." ( codeFamille,Designation,idDepot ) values (?,?,?) ";
	$params1= array($_POST['CodeFamille'],$_POST['DsgFamille'],1) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	...
        	<td><div class="etiqForm" id="" > <strong><?php echo $trad['label']['codeFamille']; ?></strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="CodeFamille"  id="CodeFamille" size="44" tabindex="1"  />
            </td>
          </tr>
			 <tr>
...
<?php $_SESSION['align'] ; ?>;resize:None;" >
</textarea>
            </td>
             </tr>
			<tr><td colspan="4"  height="20" > <div class="msgErreur">&nbsp;</div></td></tr>	  
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT codeFamille,Designation FROM familles f";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['CodeFamille']) && ($_POST['CodeFamille']!='') )
	{  
	   $sqlA .=" where codeFamille like ? " ;
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%"><?php echo $trad['label']['codeFamille']; ?> </td>
...
<!--<div id="cList">-->
	<?php
		$i=0;
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="<?php $_SESSION['align'] ; ?>"  > <?php echo $row['codeFamille']; ?> </td>
				<td align="<?php $_SESSION['align'] ; ?>" > <?php 	echo $row['Designation'];?> </td>
				<!--td align="center">
					<span class="boutons"> 
					<input type="button" title="Modifier" action="mod" class="b" onClick="modifier('<?php echo $row['codeFamille']; ?>');" />  
					</span>
			  </td>			
			  <td align="left">
				<input type="checkbox" class="checkLigne" name="<?php	echo $row['codeFamille']; ?>" value="<?php	echo $row['codeFamille']; ?>" />
			  </td-->
			  </tr>
			 <?php
			$i++;
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'famille.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['parametrage']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;<?php echo $trad['label']['Famille']; ?> &nbsp;</div>
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" > <?php echo $trad['label']['codeFamille']; ?> : </div>				</td>
					<td width="30%">
...
$(document).ready(function(){	
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'famille.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'famille.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='famille.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='famille.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
