...
function AfficheCSP(IdType,Dsg){
		...
		$('#TypeClt').val(Dsg);
		$('#BoxData').modal('hide');
		//$('#BoxMap').load("ajout_client.php?GetCsb&IdType="+IdType);	
	}
$(document).ready(function(){	
$('#BoxData').modal('show');
if(	$('#IdTypeClt').val() !=""){
...
<script language="javascript" type="text/javascript">
  ...
  function AfficheTypeClt(idActv,Dsg){
	...
	//	$('#BoxMap').load("ajout_client.php?GetTypeClt2");
		...
		if(status == google.maps.GeocoderStatus.OK) {		
		  ...
		  if(results[1]) {
			...
			for (ac = 0; ac < arrDetailAdr.length; ac++) {			
				...
				var lat=res[0];
				var longu= res[1];
				var url="ajout_client.php?add&Ville="+ville+"&Adresse="+encodeURIComponent(adressComplet)+"&Secteur="+encodeURIComponent(Secteur)+"&long="+encodeURIComponent(longu)+"&lat="+encodeURIComponent(lat);
			//	alert(url);
...
<div id="tabs" >
 ...
 <div class="form-group row">
   <div class="col-md-5 col-sm-1 form-group">
...
$('#Ville').multipleSelect({
		 ...
		 width:'100%',
		      onClick: function(view) {
				...
				$('#Secteurs').load("ajout_client.php?chargerSecteur&IdVille="+view.value);
...
// selectionner la ville du vendeur recuperer par geolocalisation
  var Villes = <?php echo json_encode($TabVilles ); ?>;
        ...
        for (var i = 0; i < Villes.length; i++) {
			...
			if(Villes[i]["DsgVille"]==encodeURIComponent("<?php echo $_GET['Ville'];?>")){		
					var idVille=Villes[i]["IdVille"];
			//alert(idVille);	
				$('#Ville').multipleSelect("setSelects", [idVille]);
				//alert("ajout_client.php?chargerSecteur&IdVille="+idVille+"&Secteur="+encodeURIComponent("<?php echo $_GET['Secteur']; ?>"));
				$('#Secteurs').load("ajout_client.php?chargerSecteur&IdVille="+idVille+"&Secteur="+encodeURIComponent("<?php echo $_GET['Secteur']; ?>"));
			}
        }	
});
//$('#Secteurs').load("ajout_client.php?chargerSecteur&IdVille=<?php  echo $IdVille ;?>&Secteur="+encodeURIComponent("<?php echo $_GET['Secteur']; ?>"));
...
$('#Localisation').click(function(){
	//	$('#BoxMap').html('<center><br/><br/>Merci de patienter pendant le chargement... <br/><img src="../images/loading2.gif" /></center>').load('ajout_client.php?getMap').dialog('open');
	 var $modal = $('#BoxData');
		var url='ajout_client.php?getMap';
    $.get(url, null, function(data) {
...
.ui-widget-header {
 ...
 <div   class="row headVente " id="infosGPS"  >
	<div style="display:table-cell; vertical-align:middle;">
		...
		&nbsp; <span style="font-size:20px;top:90%" >
		<a href="mapClient.php">
		<?php echo $trad['index']['DemarreVisite'] ; ?></a></span> 
	</div>
 </div>
...
calendrier("DateF");
$(document).ready(function() {
	//	$('#formRes').html('<center><br/><br/><?php echo $trad['msg']['Patienter'];?> <br/><img src="../images/loading2.gif" /></center>').load('ajout_client.php?getLocation');
	 var geocoder;
		...
		if(status == google.maps.GeocoderStatus.OK) {		
		  ...
		  if(results[1]) {
			...
			for (ac = 0; ac < arrDetailAdr.length; ac++) {			
				...
				var lat=res[0];
				var longu= res[1];
				var url="ajout_client.php?add&Ville="+encodeURIComponent(ville)+"&Adresse="+encodeURIComponent(adressComplet)+"&Secteur="+encodeURIComponent(Secteur)+"&long="+encodeURIComponent(longu)+"&lat="+encodeURIComponent(lat);
				
				$('#formRes').html('<center><br/><br/><?php echo $trad['msg']['Patienter'];?> <br/><img src="../images/loading2.gif" /></center>').load(url);
...
function rechercher(){
	...
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'ajout_client.php?rech'});
		}
...
function closeBoxClient(){
 ...
 function ajouter(){
		$('#act').attr('value','add');	
		/*var url='ajout_client.php?getLocation';		
...
function getActivite(){
  var $modal = $('#BoxData');
		var url='ajout_client.php?GetAct';
    $.get(url, null, function(data) {
      $modal.find('.modal-body').html(data);
    })
	
		//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("ajout_client.php?GetAct").dialog('open');
	}
function getType(open){

var $modal = $('#BoxData');
		var url='ajout_client.php?GetTypeClt2';
...
function getCSB_Clt(open){
	var $modal = $('#BoxData');
		var url='ajout_client.php?GetCsb';
	if(open!='undefined ') {
		//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("ajout_client.php?GetCsb").dialog('open');
		$.get(url, null, function(data) {
		  $modal.find('.modal-body').html(data);
		})
	}else 
...
//$('#BoxMap').html('<center><br/><br/>Merci de patienter... <br/><img src="../images/loading2.gif" /></center>').load("ajout_client.php?GetCsb");
	}
