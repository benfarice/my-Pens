...
<div Class="TitleHead"  onclick="rechercherFam() ">&nbsp;> Liste des familles <?php  //echo date("d/m/y h:i");?></div></div>
<div class="clear"></div>
							<?php 
							$key ="";
				if( (isset($_SESSION['lignesFam'])) && (count($_SESSION['lignesFam'])!=0))  {
					?>
					<div class="ListFam" ><?php	
					$i=1;
...
$k=0;
	...
	foreach($_SESSION['lignesFam'] as $u=>$v){	
		...
		?>
		  <div class="cadre " style="" onclick="AfficheSousFam('<?php echo $v['IdFamille'];?>')">
			<div  class="child"> <?php 	echo $v['DsgFamille'];?></div>
		  </div>
...
<div id="resReg"></div>
	...
	<script language="javascript" type="text/javascript">	
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'catalogue0.php?TerminerCmd',
								method			:	'post'
							});
		patienter('resReg');
		}
...
<div Class="TitleHead" >&nbsp;&nbsp;>
<span  class="Retour" onclick="AfficheFam(<?php echo $idFam;?>)">Liste des familles</span>
>
<span  class="Retour" onclick="AfficheSousFam(<?php echo $idFam;?>)">Liste des sous familles</span>
</div></div>

<div class="HeadVente">
	<div class="Left">
		<span  class="Retour" onclick="AfficheSousFam(<?php echo $idFam;?>)">Retour</span>
	</div>
	<div class="right">
	...
	foreach($groups as $u=>$v){	
	 ...
	 <li>
		...
		<div class="divLeft">
			...
			<tr><TD>Sous Famille:</td><td><?php  echo $v['dsgSousFamille'];?></td></tr>
				</table>
				...
				$key="";$c="";
					foreach($v as $r){
						...
						/*	echo "--".$key."---";
							...
							?>		
								<div class="ligne " >
									<div class=" <?php echo $c;?> divArticle " align="center"><?php  echo $r['DsgArticle'];?>					
									</div>
...
function ConsultCmd(){
	...
	$.get("catalogue0.php?VerifSession", function(response) {
	  ...
	  // verifier si le vendeur a ajouté des articles
		if(Verif==1){
		var url='catalogue0.php?ConsultCmd';	
...
$(document).ready(function(){		
	...
	$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('catalogue0.php?affFam&&VideFam');
...
function ValideCmdEtape1(){
	/*$('#formCmd').ajaxSubmit({
		target:'#res',
		url:'catalogue0.php?ValideCmd',
			method			:	'post'
		});
		patienter('res');*/
		$('#box').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('catalogue0.php?ChoixTypeReg').dialog('open');
...
function Imprimer(IdFacture){
    ...
    var TabColor = [ 
					'#ff9a87 ', '#c0ffc3 ' , '#adedf7 ', '#fcf7ab ','#fcd69f ', '#a0bffa ', '#a0fab9 ', '#fab8c3 ',
