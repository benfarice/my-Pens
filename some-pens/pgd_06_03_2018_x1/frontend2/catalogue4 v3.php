...
$Imprime.=PHP_EOL;
 ...
 $QteImp="";
	...
	$reqInser2 = "INSERT INTO  detailfactures(idFacture,[idArticle],[qte],tarif,idDepot,idFiche,ttc,UniteVente) 
			...
			$Imprime.= "Réf : ".$contenu["Reference"].PHP_EOL;
			$Imprime.=str_pad($QteImp, 5, ' ', STR_PAD_LEFT)."  X  PU:".str_pad(number_format($contenu["PriceUnite"], 2, '.', ' '), 8, ' ', STR_PAD_LEFT)."  ".str_pad(number_format($TotalPriceArti, 2, '.', ' '), 14, ' ', STR_PAD_LEFT). " DH".PHP_EOL;
			$Imprime.=PHP_EOL;
			if( $stmt3 === false ) {
...
if($error=="" ) {
   ...
   $name=date('d-m-Y H-i');
	...
	//	data: {'value':'send'},
		...
		setTimeout(function () {
			 location.href = 'mapClient.php';
      //  document.location.pathname = "mapClient.php";
    }, 1000);
	</script>
	<?php
