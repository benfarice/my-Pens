...
v.codeVendeur CodeVdr
	...
	INNER JOIN villes v2 ON v2.idville=c.ville
...
//$Imprime.=strtotime($Date." ".$Heure);
//$Imprime.=date("d/m/Y H:i", strtotime(date("Y-m-d hh:mm"))); 
...
$Imprime.=PHP_EOL;
 ...
 $QteImp="";
		if($contenu["UniteVente"]=="Box")
		{
			...
			$QteCmd=tofloat($contenu["NbrColisage"])*$contenu['Qte'];
	...
	$reqInser2 = "INSERT INTO  detailfactures(idFacture,[idArticle],[qte],tarif,idDepot,idFiche,ttc,UniteVente) 
			...
			$params2= array(
					...
					$contenu['IdArticle'],
					//$contenu['Qte'],
					tofloat(str_replace(" ","",$contenu["Qte"])),//é/ qte cmd est stockée par unité (box,palette piece)
			...
			$Imprime.=$contenu["NomArt"].PHP_EOL;
			...
			if( $stmt3 === false ) {
				...
				// calculer la qte cmdé par outer
					...
					/*else if ($contenu["UniteVente"]=="Palette"){
						$QteCmd=$_POST["NbrPalette"]*tofloat($_POST["NbrBox"])*$contenu['Qte'];
...
// on regle les facture
/*
if($TotalAvance!=0)
{
	$sqlA = "SELECT f.IdFacture, f.totalTTC FROM factures f WHERE etat='NR' AND f.idClient=".$_SESSION["IdClient"];
    $params = array();
...
//$("#txtTotal").val("<?php echo number_format("0.00", 2, '.', ' '); ?>");
	...
	$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('catalogue4.php?affMarque&&VideFam');
...
function rechercherFam(id){
		...
		var url='catalogue4.php?affFam&&Id='+id	;
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
}
function AfficheSousFam(id){
	...
	function AfficheMarque(){
		var url='catalogue4.php?affMarque';	
		$('#formRes').html('<center><br/><br/><br/><br/><img src="../images/loading2.gif" /></center>').load(url);
...
function RechArticle (){
		var Ref=$("#Ref").val();
		var url='catalogue4.php?getArticle&&Rech&&Ref='+Ref;	
		$('#boxArticle').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');
		
		}
function  AffFormRech(){
	$('#boxRech').load("catalogue4.php?FormRech").dialog('open');
}	
</script>
