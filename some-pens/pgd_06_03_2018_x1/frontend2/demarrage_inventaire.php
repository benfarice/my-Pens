       ...
       foreach($xmldata->Article as $item)
		{
		        ...
		        if($item->nbr_colisage_select != -1 || $item->nbr_piece_select != -1){
					...
					(select r.IdArticle from articles r where
                    r.Reference ='".$item->RefArticle."') 
...
$output = ' <table border="1" cellpadding="10">
		<tr>
				...
				<td colspan="6"></td>
				<td colspan="2">Heure : '.date('H:i').'</td>
		</tr>
        <tr>
...
//$file_name='C:\xampp\htdocs\PGD_thursday1\frontend2\uploads\inventaire'.$_SESSION['numero_inventaire'].'.pdf';
$file_name='\uploads\inventaire'.$_SESSION['numero_inventaire'].'.pdf';
//$file_js_name ='/frontend2/uploads/inventaire'.$_SESSION['numero_inventaire'].'.pdf';
$file_js_name ='/frontend2/uploads/inventaire'.$_SESSION['numero_inventaire'].'.pdf';
?>
<span  id="metadata-size-of-widget" title="<?php echo $file_js_name;?>"></span>
<?php
try {
     $obj_pdf = new TCPDF('P', PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);  
			       ...
			       /*  unlink('C:\xampp\htdocs\PGD_Tuesday6\upload\inventaire.pdf'); 
			          ...
			          //$obj_pdf->Output( __DIR__ .'\uploads\inventaire.pdf', 'F');
						$obj_pdf->Output(  __DIR__ .$file_name, 'F');
} catch (Exception $e) {
...
<script type="text/javascript">
	...
	var size_of_widget = "http://pgd.ma/v6" +document.getElementById("metadata-size-of-widget").title;
	console.log("console : "+size_of_widget);
	window.open(size_of_widget);
