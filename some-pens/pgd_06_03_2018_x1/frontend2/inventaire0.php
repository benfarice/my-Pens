...
<div id="resReg"></div>
	...
	<script language="javascript" type="text/javascript">	
		...
		$('#formReg').ajaxSubmit({
							target:'#resReg',
							url:'inventaire.php?TerminerCmd',
								method			:	'post'
							});
		patienter('resReg');
		}
...
//parcourir($tabArtilce);
								...
								?>
									<DIV class="haut">
										...
										<input type="radio" name="Rupture"  value="1" 
												data-labelauty="Oui|Oui" aria-label="1"	/>
										<input type="radio" name="Rupture"  value="0" 
												data-labelauty="Non|Non" aria-label="0" checked	/>
										</td>
										</tr>
										<tr>
							...
							<TD>	<input  g="date" id="NbrJourRupture" tabindex="2" name="NbrJourRupture" type="text" 
											class="form-control" 
					size="10" maxlength="10"  value=""/>	JOUR(S)
...
<div id="formRes" style="width:1278px;">
<input type="button" value="inventaire" onclick="getInventaire()">
</div>
<div class="clear"></div>
...
function ConsultCmd(){
	 var Verif="";
	$.get("inventaire.php?VerifSession", function(response) {
      Verif = response;
	  // verifier si le vendeur a ajouté des articles
		if(Verif==1){
		var url='inventaire.php?ConsultCmd';	
		$('#box').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load(url).dialog('open');
...
$("#txtTotal").val("<?php echo number_format("0.00", 2, '.', ' '); ?>");
	//$('#formRes').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire.php?affFam&&VideFam');
...
function FermerFenetre(){
	alert("fermer");
	var url='inventaire.php?FermerFenetre';	
	//$("#box").load(url);
}
function ValideCmdEtape1(){
	/*$('#formCmd').ajaxSubmit({
		target:'#res',
		url:'inventaire.php?ValideCmd',
			method			:	'post'
		});
		patienter('res');*/
		$('#box').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load('inventaire.php?ChoixTypeReg').dialog('open');
