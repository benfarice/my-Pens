...
//	$var="";
	...
	{//echo ($_GET["type"]);
		...
		window.location.href = 'type_vente.php';
		</script>
	<?php
...
<div>&nbsp;> <span  Class="TitleHead" id="DmrTrn"><?php echo $trad['index']['DemarreVisite'] ; ?></span></div>&nbsp;> <?php echo $trad['map']['map'] ; ?></div>
...
if(clt[feature.idClient])
{
	...
	//alert(activite);
	var icon="img/"+activite+".png";//alert(icon);
	marker.setIcon(icon);
}
else
{
...
//************************************************************************/
$('#boxClient').html('<center><br/><br/><img src="../images/loading2.gif" /></center>').load("mapClient.php?infoClient&idClient="+feature.idClient+"&from="+json1+"&to="+json2+"&distance="+distance+"&type="+feature.type).dialog('open');
