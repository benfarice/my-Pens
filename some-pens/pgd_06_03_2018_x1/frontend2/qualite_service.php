...
$sql = "   SELECT ville FROM clients c  WHERE c.IdClient=".$_SESSION['IdClt'];
$stmt2 = sqlsrv_query( $conn, $sql );
if( $stmt2 === false ) {
    $error.="Erreur recupération IDVILLE : ".sqlsrv_errors() . " <br/> ";
}
sqlsrv_fetch($stmt2) ;
$idVille = sqlsrv_get_field( $stmt2, 0);
$_SESSION['IdVille']=$idVille;

$dateSy = (date("d/m/Y"));
$sql="
			SELECT q.IdQuestionnaire,q.DateD+'-'+q.DateF AS Periode,
			DsgQuestion_".$lang." as DsgQuestion,
			dq.IdDetailQuestion IdQuestion
			FROM Questionnaires q
			INNER JOIN DetailsQuestionnaire dq ON q.IdQuestionnaire=dq.IdQuestionnaire
			INNER JOIN villes v ON v.idville=q.IdVille
			where v.idville = ?
			and convert(date, '".$dateSy."',105) >= convert(date, DateD,105) 
			AND convert(date, '".$dateSy."',105) <= convert(date, DateF,105) 
			and q.Etat=1 and   dq.Etat=1 and q.IdDepot=$IdDepot

";
/*
echo ($sql);
echo "<br>___".$_SESSION['IdVille'];*/
		$params = array($_SESSION['IdVille']);
		...
		{ ?>
					<div class="resAff">
						<br><br>
					<CENTER>	<?php echo $trad['msg']['AucunResultat']; ?></center>
					</div>
					<?php
		}ELSE
				{ 	
...
<?php						
	 ...
	 echo " <li><div style='text-align:center'>" ;	
		?>
		<input type="hidden" value="<?php echo $row['IdQuestionnaire'];?>" class="IdQuestionnaire"/>
		<div class="Question">
...
//chercher si la question est deja ajoutée ds la table
  ...
  function Terminer(){
	...
	var IdQuestionnaire=$(".IdQuestionnaire").val();
	  $('#res').load("qualite_service.php?goAdd&tab="+json1+"&IdQuestionnaire="+IdQuestionnaire);
					}
	});
  }
...
$Etat="";
$reqInser1 = "INSERT INTO qualite_service ([IdClt] ,[IdVendeur]  ,[Date]  ,IdDepot,IdQuestionnaire) 
				values 	(?,?,?,?,?)";
		//	echo $reqInser1;
$params1= array(
				$_SESSION['IdClt'],
