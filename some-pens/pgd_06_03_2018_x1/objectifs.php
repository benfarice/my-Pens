...
include("lang.php");
$tableInser = "departements";
$cleTable = "iddepartement";
...
$sql = "SELECT * FROM ".$tableInser." WHERE codeDepartement=? ";
$param = array($_POST['CodeObjectif']);
...
$IdAffectation = "" ;
	...
	?>
				<script type="text/javascript"> 
						alert('<?php echo $trad['msg']['codeObjectifexist']; ?>');
				</script>
	<?php
	return;
	}
	...
	$reqInser = "INSERT INTO ".$tableInser." ( codeDepartement,Designation,idVille ,iddepot) values (?,?,?,?) ";
	
	$params1= array(addslashes(mb_strtolower(securite_bdd($_POST['CodeObjectif']), 'UTF-8')),addslashes(mb_strtolower(securite_bdd($_POST['DsgObjectif']), 'UTF-8')),$_POST['Ville'],1) ;
...
<form id="formAdd" action="NULL" method="post" name="formAdd1"> 	
        ...
        <tr>
        	<td><div class="etiqForm" id="" > <strong><?php echo $trad['label']['codeObjectif']; ?></strong> : </div>
            </td>
            <td>
            <input class="FormAdd1" type="text" name="CodeObjectif"  id="CodeObjectif" tabindex="1"  style="width:220px"/>
            </td>
          </tr>
			 <tr>
          ...
          <!--  <input class="FormAdd1" type="text" name="DsgTransport" id="DsgTransport" size="25"  control="1" tabindex="6"  />-->
           <textarea  rows="1" cols="42"  tabindex="2"   class="FormAdd1" name="DsgObjectif" style="text-align:<?php echo $_SESSION['align'] ; ?>;resize:None;width:220px" >
</textarea>
            </td>
             </tr>
			 <tr>
...
if (isset($_GET['rech']) or isset($_GET['aff'])){
	...
	$sqlA = "SELECT vdr.nom+ ' '+vdr.prenom FROM 
				objectifs o
				INNER JOIN villes v ON v.idville = o.IdVille
				INNER JOIN vendeurs vdr ON vdr.idVendeur = o.IdVendeur	
	";

    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
	if(isset($_POST['CodeObjectif']) && ($_POST['CodeObjectif']!='') )
	{  
	   $sqlA .=" where Annee like ? " ;
	   $params = array("2017");
	}
	//ECHO $sqlA."<br>";
	...
	$sqlC = " ORDER BY IdObjectif desc";//$cTri $oTri ";//LIMIT $min,$max ";
...
$('#cont_pages').html('<?php echo $selPages; ?>');
	...
	<table width="70%" border="0" style="margin:auto">
      <tr class="entete">
        <td width="30%"> <?php echo $trad['label']['Ville']; ?> </td>
        <td width="50%"> <?php echo $trad['label']['Vendeur']; ?> </td>
        <td width="50%"> <?php echo $trad['label']['Objectif']; ?>  </td>		
...
<!--<div id="cList">-->
	<?php
		$i=0;
			...
			?>
			<tr  class="<?php echo $c; ?>">
				<td align="<?php echo $_SESSION['align'] ; ?>"  > <?php echo $row['codeDepartement']; ?> </td>
				<td align="<?php echo $_SESSION['align'] ; ?>" > <?php 	echo $row['objectif'];?> </td>
				...
				if(n>0){
					...
					jConfirm('Confirmer la suppression ?', null, function(r) {
						if(r)	{
							...
							$('#formSelec').ajaxSubmit({target:'#brouillon',url:'objectifs.php?delPlusieursTransports',clearForm:false});		
						}
					});
				}			
		}	
...
<div id="infosGPS" style="border-bottom:1px dashed #778; ">&nbsp;<?php echo $trad['Menu']['parametrage']; ?>&nbsp;<img src="images/tri.png" />
    &nbsp;<?php echo $trad['label']['objectif']; ?>&nbsp;</div>
		...
		<div id="formRech" style="">	
			<table width="101%" border="0" align="center" >
				  <tr>
					...
					<div class="etiqForm" id="SYMBT" ><?php echo $trad['label']['codeObjectif']; ?> : </div>				</td>
					<td width="30%">
					<!---<input class="formTop" name="COLBQ" type="hidden" size="30" value="4"/>-->
					<div align="left">
				<input class="formTop"  name="CodeObjectif" type="text" size="30" />						</div>
										</td>
...
<script language="javascript" type="text/javascript">
$(document).ready(function(){	
  			$('#formRes').load('objectifs.php?aff');
				$('#boxFamille').dialog({
					autoOpen		:	false,
					width			:	600,
					height			:	400,
	...
	function filtrer(){	
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'objectifs.php?rech',clearForm:false});
		patienter('formRes');
		return false;	
	}
function rechercher(){
		$('#formRechF').ajaxSubmit({target:'#formRes',url:'objectifs.php?rech'})
		clearForm('formRechF',0);
	}

function ajouter(){
		$('#act').attr('value','add');
		var url='objectifs.php?add';	
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}

function modifier(id){
		$('#act').attr('value','mod');
		var url='objectifs.php?mod&ID='+id;
		$('#boxFamille').html('<center><br/><br/><img src="images/loading.gif" /></center>').load(url).dialog('open');	
}
...
function terminer(){
	...
	if(act == 'mod'){ form="#formMod";} else {form="#formAdd"; }
	    $(form).validate({
                                 rules: {
                                                DsgObjectif: "required",
                                                CodeObjectif: "required",
												Ville:"required"
