...
$where=" where (EtatAvoir=0) ";
if ((isset($_POST['IdDepot'])) && ($_POST['IdDepot']!=""))
{
	$where.=" and a.iddepot=".$_POST['IdDepot'];
}
if ((isset($_POST['NumFac'])) && ($_POST['NumFac']!=""))
{
	$where.=" and NumFacture like '%".$_POST['NumFac']."%'";
		...
		if(isset($_POST['DateD']) && isset($_POST['DateF'])  )
		{
			...
			{ 	
				 $where.= " and  cast(a.DateR AS date) = convert(date, '".($_POST['DateD'])."' ,105)";					 
			}
			else
			{
				 $where.= " and  cast(a.DateR AS date)  between  convert(date, '".($_POST['DateD'])."',105) and convert(date, '".($_POST['DateF'])."' ,105)";
...
<DIV class="ListeCmd" style="">
	<table width="100%" border="0">
      <tr class="entete">
		...
		<td width="10%"><?php echo $trad['label']['TotalTTC']." (".$trad['label']['riyal'].")" ;?></td>
...
?>
	...
	>
			...
			<td  class=""  align="right"><?php echo  number_format($row['TotalTTC'], 2, '.', ' ');?>  </td>	
			<td  class=""    align="center"><?php 	$newdate = date('d/m/Y', strtotime($row['DateR']));echo $newdate; ?> </td>	
...
function rechercher(){
	...
	$('#formRechF').ajaxSubmit({target:'#formRes',url:'validation_retour.php?aff'});
		patienter('formRes');
