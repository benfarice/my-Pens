...
function ajax_func(){
        ...
        xmlhttp.onreadystatechange = function(){
            if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
                ...
                $('#date_de_a').html("Date De "+first_date+" à "+second_date);
                if($("#inputdepot option[value='"+inputdepot+"']").text() != "")
                $("#depot_de").html($("#inputdepot option[value='"+inputdepot+"']").text());
            	else
            	$("#depot_de").html("Tous");
            	if($("#inputvendeur option[value='"+inputvendeur+"']").text() != "")
                $("#le_vendeur").html($("#inputvendeur option[value='"+inputvendeur+"']").text());
            	else
            	$("#le_vendeur").html("Tous");	

            	vidange_n = $("#v_tot").html();
				gasoil_n = $("#t_gaz").html();
				autoroute_n = $("#auto_v").html();
				Divers_n = $("#divers_t").html();
				console.log(vidange_n);
				total_frais = 0 ;
				if(!isNaN(vidange_n))
					total_frais += Number(vidange_n);
				if(!isNaN(gasoil_n))
					total_frais += Number(gasoil_n);
				if(!isNaN(autoroute_n))
					total_frais += Number(autoroute_n);
				if(!isNaN(Divers_n))
					total_frais += Number(Divers_n);
				$("#title_total_frais").html(new Intl.NumberFormat("de-DE").format(total_frais));
...
<div class="row jumbotron">
	<p class="text-info col-md-12">Statistiques | Frais | <span id="date_de_a"></span>| 
		Dépôt : 
		<span id="depot_de">Tous</span> | Vendeur : 
		<span id="le_vendeur">Tous</span>
	</p>
	<div class="col-md-4"></div>
	<div class="col-md-4">
		<button class="btn btn-primary btn-block" id="another_search">Recherche</button>
	</div>
	<div class="col-md-4"></div>
...
margin: 25px 0;">
	...
	<div class="col-md-7 text-center text-info">
		Vidange
           ...
           <div class="modal-body">
		        ...
		        <div class="form-group">
			      ...
			      <select id="inputdepot" name="inputdepot" class="form-control">
			        ...
			        <?php 
			       	$query_depot = "select distinct d.Designation,d.idDepot from depots d where idDepot <> 1";
